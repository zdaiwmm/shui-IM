...
const icons = {
  ...
  people: '<svg aria-hidden="true" viewBox="0 0 24 24"><circle cx="9" cy="8" r="3"/><path d="M3 20v-1a5 5 0 0 1 5-5h2a5 5 0 0 1 5 5v1"/><circle cx="17" cy="9" r="2.4"/><path d="M21 20v-.8a4 4 0 0 0-3.2-3.9"/></svg>',
...
export class QuietRoomApp {
  ...
  private galleryRevealAll = false;
  ...
  async start(): Promise<void> {
    ...
    if (!stored) {
      ...
      await this.renderGateway({ trustedCoverActivation: true });
      return;
    }
    this.renderCover();
    void this.unreadCounter.refresh();
  ...
  private async renderGateway({ trustedCoverActivation = false, autoUnlock = trustedCoverActivation }: { trustedCoverActivation?: boolean; autoUnlock?: boolean } = {}): Promise<void> {
    ...
    if (hasVault) {
      ...
      else {const profile=await readBrowserAccessRecord() as {record?:PlatformCredentialRecord}|undefined;if(!canRender())return;if(profile?.record)this.renderBrowserAccessUnlock(profile.record);else this.renderFirstRun(null);}
    }
  }

  private gatewayTemplate(title: string, subtitle: string, content: string, compact = false, chrome: 'full' | 'plain' = 'full'): void {
    const showChrome = !compact && chrome === 'full';
    this.root.innerHTML = `
      <section class="gateway${compact ? ' gateway-gesture' : ''}${chrome === 'plain' ? ' gateway-plain' : ''}">
        ${showChrome ? `<div class="gateway-mark" aria-hidden="true">${icons.lock}</div>` : compact ? '' : `<div class="gateway-mark" aria-hidden="true">${icons.people}</div>`}
        <div class="gateway-heading">
          ${showChrome ? '<p class="eyebrow">Quiet Room</p>' : ''}
          <h1>${title}</h1>
          <p>${subtitle}</p>
        </div>
        ${content}
  ...
  private renderCorruptVault(): void {
    ...
    `);
    this.root.querySelector('#corrupt-cloud-recovery')?.addEventListener('click', () => this.transitionPage('forward', () => this.renderJointRecovery()));
  ...
  private renderRecoveryUnlock(): void {
    ...
    this.gatewayTemplate('恢复加密保险库', '输入这次恢复使用的恢复码，继续完成本设备绑定。', `
      <form class="gateway-form" id="recovery-code-form" data-recovery-keyboard>
  ...
  private mountPasskeySetup(
    ...
    verifyButton.addEventListener('click', () => {
      ...
      busy = true;
      error.textContent = '';
      // Keep the browser-owned ceremony in the trusted click stack. Busy UI
      // follows only after navigator.credentials has received that activation.
      const platformProof = preparedPlatformCredential
        ? Promise.resolve(preparedPlatformCredential)
        : createdPlatformRecord
          ? this.withDeviceVerification(async () => ({
  ...
  private clearSystemSurfaceHandoff(): void {
    if (!this.systemSurfaceHandoff) return;
    window.clearTimeout(this.systemSurfaceHandoff.timer);
    ...
    try {
      ...
      const poll=async()=>{
        ...
        try {
          ...
          if(!active())return;
          if(result.status==='approved'&&result.sealed){
            const profile=await acceptBrowserApproval(identity,requestId,keys,result.sealed);
      ...
      void poll();
    } catch(cause){this.browserAccessHold=false;if(active()){if(passkeyLookupFailed(cause)){this.showNotice(PASSKEY_UNAVAILABLE_NOTICE,'error');return;}if(!isPlatformVaultCancellation(cause)){const error=this.root.querySelector('.form-error');if(error)error.textContent=cause instanceof Error?cause.message:'接入未完成';}}}
  ...
  private renderBrowserShell(): void {
    ...
    const detail = waiting
      ? '对方进入空间后会收到授权请求。'
      : spaceWaiting ? '把邀请发给对方后，这个空间会在一小时内保留。'
  ...
  private async requestBrowserSpace(space:AccessSpace,button:HTMLButtonElement):Promise<void> {
    const current=this.browserProfile,signal=this.browserAccessAbort?.signal;if(!current||!signal||button.disabled)return;
    const verification=this.withDeviceVerification(()=>browserAccessCredential(signal,current.record));
    ...
    }catch(cause){if(!signal.aborted&&button.isConnected){const message=cause instanceof Error?cause.message:'';if(message.includes('三台设备')||message.includes('DEVICE_LIMIT'))this.showNotice(SPACE_DEVICE_LIMIT_NOTICE,'error');else if(!isPlatformVaultCancellation(cause)){const error=this.root.querySelector('.form-error');if(error)error.textContent=message||'申请未完成';}}}
  ...
  private pollBrowserSpaceAccess(signal:AbortSignal):void {
    const poll=async()=>{
      ...
      for(const space of current.profile.spaces){
        ...
        try{
          const result=await accessApi<AccessStatus>(spaceAccessPath(space.roomId,pending.proof.request.requestId),pending.token,signal);
          if(signal.aborted||this.browserProfile!==current)return;
  ...
  private async installBrowserSpace(space:AccessSpace,pending:PendingSpaceAccess,state:RoomState,signal:AbortSignal):Promise<void> {
    const current=this.browserProfile,prf=this.browserBindingProof;if(!current||!prf||!space.members||!space.role||!space.creatorFingerprint)throw new Error('本机接入凭据不完整');
    const own=state.members.find(m=>m.deviceId===pending.identity.publicBundle.deviceId);
    if(!own||own.status!=='active')throw new Error('空间加入尚未完成');
    const vault:Vault={v:3,roomId:space.roomId,accessToken:pending.token,role:space.role,pairingSecret:'',creatorFingerprint:space.creatorFingerprint,identity:pending.identity,
      members:space.members,lastSeq:own.joinSeq??0,lastReceiptSeq:own.joinReceiptSeq??0,historyUnavailableBeforeSeq:own.joinSeq??0,createdAt:new Date().toISOString(),protocol:'mls-rfc9420',pairingState:'linking',mls:{protocol:'mls-rfc9420',phase:'awaiting-welcome',lastEventSeq:space.eventSeq??0},spaceRecoveryCode:current.profile.collectionCode};
    await completeBrowserAccessJoin(vault,state,pending.proof);signal.throwIfAborted();
    await selectLocalSpace(crypto.randomUUID());signal.throwIfAborted();
    const session=await createVault(vault,'','platform',{record:current.record,prfOutput:prf.slice()},()=>!signal.aborted&&!this.privacyCovered);
    signal.throwIfAborted();session.browserAccessPrf=prf.slice();this.session=session;
    space.localId=vaultSpaceId(session.stored);delete current.profile.pending[space.roomId];
    await saveBrowserProfile(current,signal);signal.throwIfAborted();
    await rememberLocalSpace(session,undefined,{roomId:space.roomId,name:space.name});
    this.browserAccessAbort?.abort();this.browserAccessAbort=null;
    await this.openSession();
  }

  private async startBrowserAccessServices(session:VaultSession,signal:AbortSignal):Promise<void> {
    try{
      if(session.browserAccessPrf&&session.stored.unlockMethod==='platform'){
    ...
    const poll=async()=>{
      ...
      try{
        ...
        let latest=0;
        for(const item of response.requests){const id=item.proof.request.requestId;const deadline=Math.min(deadlines.get(id)??Infinity,accessDeadline(item.expiresAt,response.serverTime));deadlines.set(id,deadline);latest=Math.max(latest,deadline);}
        if(latest>performance.now())this.peerAccessHints.set(session.vault.roomId,latest);else this.peerAccessHints.delete(session.vault.roomId);
        if(this.accessDialog?.id.startsWith('space:')&&!response.requests.some(r=>`space:${r.proof.request.requestId}`===this.accessDialog?.id))this.accessDialog.close();
        const next=response.requests.find(r=>!dismissed.has(r.proof.request.requestId)&&((deadlines.get(r.proof.request.requestId)??0)>performance.now()));
        if(next&&!this.accessDialog&&!this.root.querySelector('[role="dialog"]')){dismissed.add(next.proof.request.requestId);await this.showPeerAccess(next,deadlines.get(next.proof.request.requestId)!,signal);}
        const mailboxes=await preparedMailboxes(session);signal.throwIfAborted();
        if(mailboxes.length){
          const mail=await accessApi<{results:{id:string;requests:BrowserRequest[];serverTime:string}[]}>('/api/browser-access-poll','',signal,'POST',{mailboxes:mailboxes.map(m=>({id:m.id,token:m.token}))});
          for(const result of mail.results){const mailbox=mailboxes.find(m=>m.id===result.id)!;
            const request=result.requests.find(r=>!dismissed.has(r.requestId));
            if(request&&!this.accessDialog&&!this.root.querySelector('[role="dialog"]')){
              dismissed.add(request.requestId);const code=await accessSafetyCode({requestId:request.requestId,browserId:request.browserId,browserKey:request.browserKey});signal.throwIfAborted();
              const id=`browser:${request.requestId}`;
              const close=mountAccessApproval(this.root,{signal,title:'允许此浏览器继续使用？',description:'仅认可新浏览器并提供空间列表，各空间仍需对方单独授权。',code,deadline:accessDeadline(request.expiresAt,result.serverTime),
                approve:()=>approveBrowser(session,mailbox,request,signal),reject:async()=>{await accessApi(mailboxPath(mailbox,request.requestId),mailbox.token,signal,'DELETE');},closed:()=>{if(this.accessDialog?.id===id)this.accessDialog=null;}});
              this.accessDialog={id,close};break;
            }
          }
        }
      }catch{ /* Offline requests keep their original local deadlines. */ }
      if(!signal.aborted)window.setTimeout(()=>void poll(),3000);
    };void poll();
  }

  private async showPeerAccess(request:PeerAccessRequest,deadline:number,signal:AbortSignal):Promise<void> {
    const session=this.session;if(!session||signal.aborted||deadline<=performance.now())return;
    const source=session.vault.members.find(m=>m.deviceId===request.proof.certificate.sourceDeviceId);
    if(!source||source.role===session.vault.role||!await verifyAccessProof(request.proof,session.vault.roomId,source))return;
    const code=await accessSafetyCode(request.proof);if(signal.aborted)return;
    const id=`space:${request.proof.request.requestId}`;
    const close=mountAccessApproval(this.root,{signal,title:'允许对方的新设备加入？',description:`授权后，对方可在新设备中使用「${this.currentSpaceName}」。加入前的历史不会同步。`,code,deadline,
      approve:async()=>{
        await withVaultMutation(session,async mutation=>{
          signal.throwIfAborted();
          const existing=session.vault.mls?.pendingMembership;
          if(existing&&existing.event.targetId!==request.target.deviceId)throw new Error('另一项设备变更尚未完成');
          const pending=existing??await prepareMlsMembership(session.vault,'add',request.target,request.proof);
          session.vault.mls!.pendingMembership=pending;await saveVault(session,mutation);signal.throwIfAborted();
          try{const result=await publishMlsMembership(session.vault.roomId,session.vault.accessToken,pending.event);signal.throwIfAborted();await this.applyRoomState(result.state,mutation);}
          catch(cause){if(cause instanceof ApiError&&!cause.retryable){session.vault.mls!.pendingMembership=undefined;await saveVault(session,mutation);}throw cause;}
        });
        this.peerAccessRequests=this.peerAccessRequests.filter(r=>r!==request);this.peerAccessHints.delete(session.vault.roomId);
      },
      reject:async()=>{await accessApi(spaceAccessPath(session.vault.roomId,request.proof.request.requestId),session.vault.accessToken,signal,'DELETE');this.peerAccessRequests=this.peerAccessRequests.filter(r=>r!==request);this.peerAccessHints.delete(session.vault.roomId);},
      closed:()=>{if(this.accessDialog?.id===id)this.accessDialog=null;}});
    this.accessDialog={id,close};
  }

  private renderFirstRun(invite: Invite | null): void {
    if (location.hash === '#legacy-recovery') { this.renderCloudRecovery(); return; }
    if (invite) {
  ...
  private async handleCreate(platformResult: PlatformCredentialResult): Promise<void> {
    ...
    try {
      ...
      const vault: Vault = {
        ...
        lastReceiptSeq: 0,
        pairingState: 'ready',
        recoveryExperience: { welcomePending: true, coverEnabled: false },
        createdAt: room.createdAt,
  ...
  private showRecoveryAuthorization(request: RecoveryRequest, state: RoomState): void {
    ...
    if (this.root.querySelector(`.recovery-authorization-sheet[data-request-id="${CSS.escape(request.requestId)}"]`)) return;
    ...
    sheet.dataset.requestId = request.requestId;
  ...
  private async openSession(): Promise<void> {
    ...
    if (invitation.kind !== 'invalid') {
      if (session.vault.roomId !== invitation.invite.roomId) {
        ...
        if (target) { await this.switchPrivateSpace(target); return; }
        await this.beginNewSpace(invitation);
        return;
      }
      history.replaceState(null, '', location.pathname + location.search);
    }
    ...
    if (!session.vault.recoveryExperience?.completed && (session.vault.recoverySource || session.vault.backup?.newCodePending)) {
    ...
    if (!this.isRuntimeActive(epoch, session)) return;
    if (location.hash === '#legacy-backup') { this.renderBackupSettings(); this.startAutomaticBackup(); return; }
  ...
  private async applyRoomState(state: RoomState, mutation: VaultMutation): Promise<void> {
    ...
    if (!this.isRuntimeActive(epoch, session)) return;
    const acceptedMlsSeq = session.vault.mls?.lastEventSeq ?? null;
    if (localProtocol === 'mls-rfc9420' && acceptedMlsSeq !== null && (state.nextMlsEventSeq ?? 0) < acceptedMlsSeq) {
      return;
    }
    if (
    ...
    shield?.classList.toggle('needs-preparation', !(session.vault.backup && session.vault.recoveryExperience.codeSaved === session.vault.backup.id && session.vault.recoveryExperience.peerPrepared));
  ...
  private async handleMembership(state: RoomState): Promise<void> {
    ...
    try {
      ...
      if (!this.isRuntimeActive(epoch, session)) return;
      this.paintInviteProgress(state.invitationProgress?.stage);
  ...
  private invitePeerCopy(stage?: string): { label: string; badge: string } {
    if (stage === 'setting') return { label: '对方正在设置访问密钥', badge: '设置中' };
    if (stage === 'opened') return { label: '对方已打开，正在确认加入', badge: '已打开' };
    return { label: '等待对方打开邀请', badge: '待打开' };
  }

  private paintInviteProgress(stage?: string): void {
    const peer = this.invitePeerCopy(stage);
    const progress = this.root.querySelector<HTMLElement>('#invitation-progress');
    const badge = this.root.querySelector<HTMLElement>('#invite-peer-badge');
    if (progress) progress.textContent = peer.label;
    if (badge) badge.textContent = peer.badge;
  }

  private paintQrCanvas(canvas: HTMLCanvasElement, value: string): void {
  ...
  private captureUnlockResume(): void {
    if (this.privacyCovered) return;
    if (this.imagePickerActive || this.filePickerActive || this.fileExportActive
      || this.microphonePromptActive || this.callPermissionActive || this.systemSurfaceTokens.size > 0) {
      this.unlockResume = null;
      return;
    }
    ...
    this.unlockResume = root.querySelector('.cover-practice-page') ? 'cover-practice'
      ...
      : root.querySelector('.chat-shell') ? 'chat'
      : null;
  }

  private resumeUnlockedPage(): void {
    const resume = this.unlockResume;
    this.unlockResume = null;
    if (resume === 'recovery-center') this.renderRecoveryCenter();
    else if (resume === 'cover-practice') this.renderCoverPractice();
    else if (resume === 'local-backup') this.renderLocalHistoryBackup('export');
    else if (resume === 'local-backup-import') this.renderLocalHistoryBackup('import');
    else if (resume === 'gallery') this.renderGallery();
  ...
  private async switchPrivateSpace(space: PrivateSpace): Promise<void> {
    ...
    if (held && stored.unlockMethod === 'platform' && stored.v === 3 && stored.platform.credentialId === held.record.credentialId) {
      const unlocked = await unlockVault('', Promise.resolve(held.prfOutput.slice()));
      if (this.privacyCovered) { releaseDeviceCredential(unlocked); return; }
      unlocked.browserAccessPrf = held.browserAccessPrf?.slice();
      this.rememberDeviceCredential(unlocked);
      this.session = unlocked;
      await this.openSession();
      return;
  ...
  private async returnToSpace(): Promise<void> {
    const id = this.returnSpaceId;
    if (!id) return;
    history.replaceState(null, '', location.pathname + location.search);
    await this.switchPrivateSpace({ roomId: '', name: '', localId: id });
  }

  private async beginNewSpace(invitation?: Exclude<ReturnType<typeof classifyInviteHash>, { kind: 'invalid' }>): Promise<void> {
  ...
  private renderChat(): void {
    ...
    this.root.innerHTML = `
      <section class="chat-shell">
        <header class="chat-header" data-presence-style="${readPresenceStyle()}">
          ...
          <span class="space-header-balance" aria-hidden="true"></span>
        </header>
        <div class="system-notices" aria-label="本机安全提醒">
          ...
          ${this.session.vault.recoveryExperience?.completed === 'recovered' && !this.uiPreferences.historyImportDismissed ? `
            <aside class="pinned-notice" id="history-import-banner">
              ...
              <button type="button" id="dismiss-history-import" aria-label="关闭导入提醒">${icons.close}</button>
            </aside>
          ` : ''}
    ...
    let rawCommitPending = false;
    let lastCompositionEndAt = 0;
    let lastCompositionEndValue = '';
    const commitRawComposition = () => {
      if (!rawCommitPending || activeComposition || !ownsActiveChat()) return;
      rawCommitPending = false;
      const current = textarea.value;
      if (!current.startsWith(compositionBefore) || !current.endsWith(compositionAfter)) return;
      const currentEnd = current.length - compositionAfter.length;
    ...
    textarea.addEventListener('compositionupdate', event => {
      // Keep the phonetic preedit before the IME replaces it with a candidate.
    ...
    textarea.addEventListener('keydown', (event) => {
      ...
      if (activeComposition || event.isComposing || event.keyCode === 229
        ...
        event.preventDefault();
        rawCommitPending = true;
        requestAnimationFrame(commitRawComposition);
        return;
      }
      event.preventDefault();
    ...
    this.root.querySelector('#import-own-history')?.addEventListener('click', () => this.transitionPage('forward', () => this.renderLocalHistoryBackup('import')));
    this.root.querySelector('#dismiss-history-import')?.addEventListener('click', () => {
      this.uiPreferences.historyImportDismissed = true;
      this.flushUiPreferencesSave();
      this.root.querySelector('#history-import-banner')?.remove();
    });
  ...
  private mountChatLayout(): void {
    ...
    this.chatBottomControl = mountChatBottomControl({
      ...
      list,
      latest: () => this.chatLatestTimelineRow(),
  ...
  private async showDeviceInvite(invite: DeviceInvite | RepairInvite, session = this.session, epoch = this.runtimeEpoch, returnFocus?: HTMLElement): Promise<void> {
    ...
    sheet.querySelector('[data-copy]')?.addEventListener('click', async () => {
      try {
        ...
        if (!this.isRuntimeActive(epoch, session) || !sheet.isConnected || generation !== this.deviceInviteGeneration) return;
        input.select();
        if (document.execCommand('copy')) this.showNotice('链接已复制');
        else this.showNotice('请选择链接后复制', 'error');
      }
    });
    try {
  ...
  private chatLatestTimelineRow(): HTMLElement | undefined {
    const last = this.chatLayoutElements?.list.lastElementChild;
    return last instanceof HTMLElement ? last : this.renderedMessageOrder.at(-1);
  }

  private chatBottomScrollTop(): number {
    const chat = this.chatLayoutElements;
    const latest = this.chatLatestTimelineRow();
    if (!chat?.shell.isConnected || !latest?.isConnected) return 0;
    const marginBottom = Number.parseFloat(getComputedStyle(latest).marginBottom) || 0;
    // Use the newest timeline row and composer edges, not document extent: the
    // latter can include a short-history minimum height or a stale Safari layout
    // viewport. Local system cards sit after bubbles and still count as bottom.
    return Math.max(0, this.chatScrollTop + latest.getBoundingClientRect().bottom + marginBottom
      - this.chatComposerLayoutTop(chat.composer) + CHAT_LATEST_GAP);
  }
  ...
  private beginNativeKeyboardDismiss(height = this.chatViewportHeight): void {
    const chat = this.chatLayoutElements;
    const row = this.chatLatestTimelineRow();
  ...
  private alignNativeChatContents(): boolean {
    ...
    const openingContent = this.nativeKeyboardOpening && delta > 1
      ? this.chatLatestTimelineRow()?.firstElementChild : null;
  ...
  private async sendPayloadLocked(payload: MessagePayload, existingClientMsgId: string | undefined, mutation: VaultMutation): Promise<void> {
    ...
    if (session.vault.protocol === 'mls-rfc9420' && session.vault.mls?.phase !== 'active') {
      throw new Error('私密空间尚未建立完成，内容不会上传或发送');
    }
    const epoch = this.runtimeEpoch;
  ...
  private async processImageBatch(files: File[], destination: 'chat' | 'gallery', operationSignal?: AbortSignal, expression = false, mediaRetryId?: string, expressionAutoHide = false): Promise<boolean> {
    ...
    if (session.vault.protocol === 'mls-rfc9420' && session.vault.mls?.phase !== 'active') {
      this.showNotice('私密空间尚未建立完成，文件不会上传', 'error');
      return false;
    }
    if (destination === 'gallery' && session.vault.role !== 'creator') {
      this.showNotice('只有私密空间创建者可以向保险箱上传文件', 'error');
      return false;
    }
    const epoch = this.runtimeEpoch;
    ...
    try {
      ...
      for (const [index, file] of files.entries()) {
        ...
        const uploadCallbacks: Parameters<typeof encryptImageFile>[1] = {
          ...
          reserve: (blobId, chunkCount, encryptedSize) => this.retryOperation(
            () => reserveBlob(vault.roomId, vault.accessToken, blobId, chunkCount, encryptedSize, signal), 3, signal),
          status: (blobId) => this.retryOperation(
            () => getBlobStatus(vault.roomId, vault.accessToken, blobId, signal), 3, signal),
          ...
          complete: (blobId) => this.retryOperation(
            () => completeBlob(vault.roomId, vault.accessToken, blobId, signal), 3, signal),
          savePlan: async (plan) => {
  ...
  private createEntranceGuideCard(): HTMLElement | null {
    ...
    if (this.entranceGuideCard) return this.entranceGuideCard;
  ...
  private renderMessages({ scroll = 'preserve' }: { scroll?: 'preserve' | 'position' | 'bottom' | 'send' | 'restore' } = {}): void {
    ...
    const entranceCard = this.createEntranceGuideCard();
    ...
    if (messages.length === 0 && this.mediaUploads.size === 0) {
      ...
      this.renderedMessageDates.clear();
      this.renderedMessageSeq.clear();
      list.replaceChildren(empty, ...(entranceCard ? [entranceCard] : []));
    } else {
  ...
  private renderRecoveryRotation(): void {
    ...
    if (!session || this.privacyCovered) return;
    const epoch = this.runtimeEpoch;
    this.gatewayTemplate('更新恢复保护', '正在用新恢复码保护私密空间和历史备份。此步骤不会读取历史消息或保险箱。', `
    ...
    const complete = async () => {
      ...
      try {
        ...
        await syncSpaceDirectory(session, this.runtimeAbort!.signal);
        if (!this.isRuntimeActive(epoch, session)) return;
        this.gatewayTemplate('恢复保护已更新', '你的空间恢复码保持不变，空间内的设备恢复凭据已更新。', `
          <code class="local-recovery-code"></code><p class="field-hint">恢复码已在本机加密保管，也请单独保存一份。历史内容尚未载入。</p>
          <button class="primary-button" id="confirm-new-recovery" type="button">我已保存，进入私密空间</button>
  ...
  private runAutomaticBackup(force = false): Promise<void> {
    ...
    const epoch = this.runtimeEpoch;
    const run = syncCloudBackup(session, signal, { force }).then(() => syncSpaceDirectory(session, signal)).then(() => {
      if (this.isRuntimeActive(epoch, session)) {
        this.backupError = '';
        void fetch(`/api/rooms/${session.vault.roomId}/preparation`, { method: 'POST', signal, headers: { Authorization: `Bearer ${session.vault.accessToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ saved: Boolean(session.vault.backup && session.vault.recoveryExperience?.codeSaved === session.vault.backup.id) }) }).catch(() => undefined);
      }
    }).catch(cause => {
  ...
  private renderBackupSettings(): void {
    ...
    this.root.querySelector('#backup-back')?.addEventListener('click', () => {
      ...
      if (location.hash === '#legacy-backup') history.replaceState(null, '', `${location.pathname}${location.search}`);
  ...
  private async revealLocalRecoveryCode(unlocked: VaultSession): Promise<void> {
    ...
    this.session = live;
    this.runtimeAbort ??= new AbortController();
    await syncSpaceDirectory(live, this.runtimeAbort.signal);
    if (this.privacyCovered || this.session !== live) return;
    this.resetIdleLock();
    this.privacyCovered = false;
    document.body.className = 'app-mode';
  ...
  private confirmDisableCover(): void {
    const session = this.session;
    if (!session || this.privacyCovered) return;
    this.root.querySelector('.more-menu')?.removeAttribute('open');
    if (this.root.querySelector('#disable-cover-dialog')) return;
    const dialog = document.createElement('div');
    dialog.className = 'confirm-overlay';
    dialog.id = 'disable-cover-dialog';
    dialog.setAttribute('role', 'dialog');
    dialog.setAttribute('aria-modal', 'true');
    ...
    dialog.querySelector('#disable-cover-anyway')!.addEventListener('click', () => {
      ...
      void withVaultMutation(session, async mutation => {
        session.vault.recoveryExperience = { ...session.vault.recoveryExperience, coverEnabled: false };
        await saveVault(session, mutation);
      }).then(() => {
        localStorage.setItem('quiet-room:cover-enabled', '0');
        if (!this.privacyCovered && this.session === session) {
          this.renderChat();
          this.showNotice('关闭成功');
        }
      }).catch(() => this.showNotice('设置未保存，请重试', 'error'));
    });
  }
  ...
  private openJointRecoveryCodeDialog(link: JointLink | null, helper: VaultSession | null): void {
    ...
    sheet.setAttribute('role', 'dialog');
    sheet.setAttribute('aria-modal', 'true');
    sheet.setAttribute('aria-label', '输入恢复码');
    sheet.setAttribute('data-recovery-keyboard', '');
    sheet.innerHTML = `<form class="confirm-dialog recovery-code-panel" id="joint-code-form">
      <div class="recovery-code-heading">
        <span></span>
        <h2>输入恢复码</h2>
        <button class="icon-button" id="joint-code-close" type="button" aria-label="关闭">${icons.close}</button>
      </div>
    ...
    form.addEventListener('submit', async event => {
      ...
      try {
        if (code.startsWith('QR4-')) {
          ...
          const select = document.createElement('select'); select.name = 'space'; select.className = 'recovery-flow-input';
          recoveryChoices.forEach((space, index) => { const option = document.createElement('option'); option.value = String(index); option.textContent = space.name; select.append(option); });
          choices.append(select); input.closest('label')!.after(choices); button.textContent = '恢复此空间'; select.focus();
          return;
        }
  ...
  private renderJointProgress(): void {
    const session = this.session, pending = session?.vault.pendingJointRecovery;
    if (!session || !pending || !this.runtimeAbort || this.privacyCovered) return;
    ...
    this.root.innerHTML = `<main class="recovery-flow-page joint-waiting-page">
      ...
      <section class="recovery-flow-content joint-progress" id="joint-progress">
        ...
        <div class="recovery-flow-group" aria-live="polite">
          ...
          <div class="recovery-flow-row"><span><strong>对方参与</strong><small id="joint-participant-detail">等待对方打开恢复链接</small></span><em class="recovery-status-badge" id="joint-participant-badge">等待中</em></div>
        </div>
        <aside class="recovery-flow-notice warn" id="joint-scope-change" hidden><strong>请确认调整</strong><p>对方调整了需要恢复的人。请核对上方对象和设备，再确认本次请求。</p></aside>
    ...
    const button = this.root.querySelector<HTMLButtonElement>('#joint-approve')!, error = host.querySelector<HTMLElement>('.form-error')!;
    const copy = this.root.querySelector<HTMLButtonElement>('#joint-copy')!;
    ...
    const finish = async (next: JointSnapshot) => {
      ...
      await completeJointRecovery(session, next, signal);
      if (!active()) return;
      await this.openSession();
  ...
  private showJointRecoveryCompletion(kind: 'helper' | 'recovered'): void {
    const session = this.session;
    if (!session || this.privacyCovered || !this.root.querySelector('.chat-shell')) return;
    const dialog = document.createElement('div');
    dialog.className = 'confirm-overlay';
    ...
    dialog.querySelector('#joint-import-history')?.addEventListener('click', () => { closeDialog(dialog); this.renderLocalHistoryBackup('import'); });
    dialog.querySelector('#joint-save-new-code')?.addEventListener('click', () => { closeDialog(dialog); this.renderRecoveryCenter(); });
  }

  private renderRecoveryCenter(): void {
    ...
    this.root.querySelector('#save-my-code')!.addEventListener('click', async () => {
      ...
      button.disabled = true;
      try {
        await this.runAutomaticBackup(true);
  ...
  private renderCoverPractice(): void {
    ...
    this.root.innerHTML = `<main class="cover-practice-page">
      ...
      <section class="cover" aria-label="遮蔽演练。内容已隐藏，遮蔽本身不是身份验证。">
        ...
        <p class="cover-practice-notice">离开私密空间后（浏览器进后台或锁屏等）会自动进入遮蔽层保护隐私。</p>
        <button class="cover-trigger practice-hotspot" type="button" aria-label="长按一秒练习返回"><span>长按</span></button>
      </section>
    </main>`;
    const hold = this.root.querySelector<HTMLButtonElement>('.cover-trigger')!;
    let holdTimer: number | undefined;
    let learned = false;
    const save = async (enabled: boolean) => { await withVaultMutation(session, async mutation => { session.vault.recoveryExperience = { ...session.vault.recoveryExperience, coverEnabled: enabled }; await saveVault(session, mutation); }); localStorage.setItem('quiet-room:cover-enabled', enabled ? '1' : '0'); this.renderChat(); };
    const showLearned = () => {
      if (learned || !hold.isConnected || this.privacyCovered) return;
      learned = true;
      const dialog = document.createElement('div');
      dialog.className = 'confirm-overlay';
      dialog.setAttribute('role', 'dialog');
      ...
      dialog.querySelector('#skip-cover')!.addEventListener('click', () => void save(false));
    };
    const begin = () => { window.clearTimeout(holdTimer); hold.classList.add('is-holding'); holdTimer = window.setTimeout(() => { if (!hold.isConnected || this.privacyCovered) return; hold.classList.remove('is-holding'); showLearned(); }, 1000); };
    const cancel = () => { window.clearTimeout(holdTimer); hold.classList.remove('is-holding'); };
    hold.addEventListener('pointerdown', event => { if (event.button === 0) begin(); });
    hold.addEventListener('pointerup', cancel); hold.addEventListener('pointercancel', cancel); hold.addEventListener('pointerleave', cancel);
    hold.addEventListener('keydown', event => { if ((event.key === ' ' || event.key === 'Enter') && !event.repeat) begin(); }); hold.addEventListener('keyup', cancel);
    this.runtimeAbort?.signal.addEventListener('abort', cancel, { once: true });
  ...
  private renderEntranceCard(): void {
    ...
    this.root.innerHTML = `<main class="recovery-flow-page">
      ...
      <section class="recovery-flow-content">
        <div class="recovery-flow-hero">${icons.file}</div>
        <h1>下次，一点就到</h1>
        <p class="recovery-flow-lead">不用记住网址。选择你愿意留下的入口。</p>
        <div class="recovery-flow-group">
          <button class="recovery-flow-row" id="entry-home" type="button"><span><strong>添加到主屏幕</strong><small>方便常用，会出现桌面图标</small></span><b aria-hidden="true">›</b></button>
    ...
    this.root.querySelector('#entrance-done')!.addEventListener('click', returnToChat);
    this.root.querySelector('#entrance-skip')!.addEventListener('click', returnToChat);
  }

  private renderCloudRecovery(): void {
    this.gatewayTemplate('恢复已有私密空间', '输入恢复码，自动取得服务器上的加密备份并在本机解密。', `
  ...
  private renderViewerVideo(
    ...
    const safeInline = Boolean(onDelete);
    const setControlsVisible = (visible: boolean) => {
      if (safeInline && live()) viewer.dataset.videoControls = visible ? 'visible' : 'hidden';
    };
    ...
    viewer.querySelector<HTMLElement>('.viewer-stage')!.addEventListener('click', event => {
      if (!safeInline || video.paused || event.target instanceof Element && event.target.closest('button, input')) return;
      setControlsVisible(viewer.dataset.videoControls === 'hidden');
    }, { signal: events.signal });
    ...
    const cleanup = () => {
      active = false;
      delete viewer.dataset.videoControls;
      deleteCleanup?.();
      events.abort();
      if (nativePendingTimer !== null) window.clearTimeout(nativePendingTimer);
  ...
  private mountGalleryCurationActions(source: HTMLElement, target: GalleryCurationTarget, tab: GalleryTab, countKey: string, manifest: ImageManifest,
    ...
    const open = (fromPointerHold = false) => {
      ...
      this.openGalleryCurationActions(source, target, tab, countKey, manifest, openDetails, onSaved);
    };
  ...
  private openGalleryCurationActions(source: HTMLElement, target: GalleryCurationTarget, tab: GalleryTab, countKey: string, manifest: ImageManifest,
    openDetails?: () => void, onSaved?: (action: 'pin' | 'unpin' | 'hide') => boolean): void {
    const session = this.session;
    const epoch = this.runtimeEpoch;
    ...
    const dialog = mountDialog(sheet, {
      ...
      signal: this.runtimeAbort?.signal,
      returnFocus: source,
      initialFocus: sheet.querySelector<HTMLButtonElement>('[data-gallery-action="send"], [data-gallery-action="pin"]'),
    });
    ...
    let saving = false;
    sheet.querySelector('[data-gallery-action="send"]')?.addEventListener('click', () => {
      if (saving || !this.isRuntimeActive(epoch, session)) return;
      saving = true;
      ...
      void (async () => {
        try {
          ...
          const payload: MessagePayload = target.category === 'files' || isVideoFile(manifest)
            ? { v: 1, kind: 'file', file: manifest, sentAt }
            : { v: 1, kind: 'image', image: manifest, sentAt };
          await this.enqueuePayload(payload);
          if (!this.isRuntimeActive(epoch, session)) return;
          dialog.close({ animate: false, restoreFocus: false });
          this.renderChat();
          ...
          this.operationalError(cause, '未能发送至聊天，保险箱原项目已保留');
        }
      })();
    });
  ...
  private renderGallery(tab: GalleryTab = 'images'): void {
    ...
    // image cache. Later pages inherit a visit-wide reveal choice.
    if (!this.root.querySelector('.gallery-shell')) {
      this.galleryRevealedAssets.clear();
      this.galleryRevealAll = false;
    }
    ...
    const allImagesRevealed = () => assets.length > 0 && (this.galleryRevealAll || assets.every(asset =>
    ...
    const updateTileVisibility = (button: HTMLButtonElement, key: string, manifest: ImageManifest, animate = true) => {
      const revealed = this.galleryRevealAll || this.galleryRevealedAssets.has(key);
    ...
    footer.innerHTML = `<p role="status" class="gallery-scan-status">正在查找${category}…</p>`;
    ...
    const addMessages = (messages: DecryptedMessage[]) => {
      ...
      for (const message of messages) {
        ...
        if ((message.payload.kind === 'file' || message.payload.kind === 'gallery-file') && !isVideoFile(message.payload.file)) {
          ...
          const fileManifest = message.payload.file;
          ...
          this.mountGalleryCurationActions(button, target, tab, key, fileManifest, undefined,
          ...
          if (favorites) button.addEventListener('click', () => {
            ...
            if (Date.now() < this.suppressMediaClickUntil) return;
            this.openGalleryCurationActions(button, target, tab, key, fileManifest);
          });
        ...
        for (const [assetIndex, manifest] of manifests.entries()) {
          ...
          button.addEventListener('click', () => {
            ...
            if (!this.galleryRevealAll && !this.galleryRevealedAssets.has(key)) {
          ...
          this.mountGalleryCurationActions(button, target, tab, key, manifest, video || favorites ? undefined : () => {
            if (!this.isRuntimeActive(epoch, session) || !button.isConnected) return;
            this.galleryScrollTop.images = grid.scrollTop;
            const currentIndex = assets.findIndex(asset => asset.clientMsgId === message.clientMsgId && asset.assetIndex === assetIndex);
            if (currentIndex >= 0) this.openImageViewer(
    ...
    const loadMore = async () => {
      ...
      try {
        ...
        do {
          ...
          if (assets.length || fileCount) clearPlaceholders();
          updateCounts();
        ...
        updateCounts();
        status.textContent = hasMore ? '上拉继续加载' : assets.length + fileCount ? `已加载本机保存的全部${category}`
        ...
        if (initial) {
          ...
          initial = false;
          if (!favorites) window.setTimeout(() => {
            if (grid.isConnected && this.isRuntimeActive(epoch, session)) void this.primeGalleryCounts(session, epoch, signal);
          }, 500);
        }
      } catch (cause) {
        if (this.isRuntimeActive(epoch, session) && grid.isConnected) {
          clearPlaceholders();
          status.textContent = '保险箱记录暂时无法读取，上拉重试';
    ...
    const atEnd = () => grid.scrollTop + grid.clientHeight >= grid.scrollHeight - 64;
    grid.addEventListener('scroll', () => {
      if (atEnd()) void loadMore();
    }, { passive: true });
    grid.addEventListener('wheel', event => {
      if (event.deltaY > 0 && atEnd()) void loadMore();
    }, { passive: true });
    ...
    grid.addEventListener('touchend', event => {
      ...
      if (touchStartY !== null && endY !== undefined && touchStartY - endY > 40 && atEnd()) void loadMore();
      touchStartY = null;
    }, { passive: true });
    grid.addEventListener('keydown', event => {
      if ((event.key === 'ArrowDown' || event.key === 'PageDown' || event.key === 'End') && atEnd()) void loadMore();
    });
    void loadMore();
  }
  ...
  private async renderImageDetail(manifest: ImageManifest): Promise<void> {
    ...
    if (session.vault.role !== 'creator') {
      this.renderChat();
      this.showNotice('保险箱仅对私密空间创建者开放', 'error');
      return;
    }
    const epoch = this.runtimeEpoch;
    this.root.innerHTML = `
  ...
  private ensureAppToast(): HTMLElement {
    let toast = document.getElementById('app-toast');
    if (!toast) {
      toast = document.createElement('div');
      toast.id = 'app-toast';
      toast.className = 'notice app-toast';
  ...
  private async adoptHeldDeviceCredential(session: VaultSession): Promise<void> {
    ...
    session.browserAccessPrf?.fill(0);
  ...
  private cleanupRuntime(preserveFilePicker = false): void {
    ...
    if (this.session) releaseDeviceCredential(this.session);
    this.browserProfile=null;this.peerAccessRequests=[];
  ...
  private updateCallControls(): void {
    ...
    else if (vault?.protocol !== 'mls-rfc9420' || vault.mls?.phase !== 'active') unavailableReason = '私密空间建立后可发起通话';
    ...
    for (const button of this.root.querySelectorAll<HTMLButtonElement>('#start-video-call, #start-audio-call')) {
      ...
      button.title = ready ? (button.id === 'start-video-call' ? '视频通话' : '语音通话') : '双方打开最新版并连接私密空间后可通话';
