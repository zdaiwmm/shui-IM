...
export function mountSpaceDrawer(root: HTMLElement, options: {
  ...
  function row(s: PrivateSpace) {
    ...
    return `<button class="space-row ${selected ? 'is-selected' : ''}" data-space="${i}" aria-current="${selected ? 'true' : 'false'}"><span class="space-row-copy">${selected ? '<small class="space-current-label">当前空间</small>' : ''}<strong>${escape(s.name)}</strong>${selected ? '' : `<small class="space-message-preview ${s.waiting ? 'is-waiting' : ''}" ${s.preview ? 'title="本机上次打开时的消息"' : ''}>${escape(s.waiting ? '等待对方加入' : s.preview || '打开空间查看消息')}</small>`}</span>${selected ? `<span class="space-selected-check" aria-hidden="true">${check}</span>` : `<span class="space-row-trailing"><span class="space-unread" data-unread="${i}" hidden></span><span class="space-row-arrow" aria-hidden="true">${arrow}</span></span>`}</button>`;
  }
  function list() {
