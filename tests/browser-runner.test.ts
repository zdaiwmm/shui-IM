...
describe('browser regression groups', () => {
  it('partitions group 2 into disjoint concurrent runner shards without omissions', async () => {
    ...
    expect(new Set(shards.flat()).size).toBe(38);
    expect(shards.map(shard => shard.length)).toEqual([10, 10, 9, 9]);
  ...
  it('partitions every existing browser entry exactly once and preserves full-suite order', async () => {
    const expected = [
      ...
      'unread-counter', 'reaction-history', 'message-deletion', 'vault-lifecycle', 'spaces', 'space-drawer-ui', 'voice-lifecycle', 'voice-submission',
    ...
    expect(new Set(all).size).toBe(39);
  ...
  it('executes the full local suite sequentially and records every script and the total duration', async () => {
    ...
    expect(log.mock.calls.filter(([line]) => line.startsWith('[browser] PASS'))).toHaveLength(39);
    expect(log).toHaveBeenLastCalledWith('[browser] TOTAL 3.90s; 39 passed, 0 failed, 0 not run.');
  });
  ...
  it('stops a failing group, preserves its failure and explicitly reports scripts that did not run', async () => {
    ...
    expect(log).toHaveBeenLastCalledWith('[browser] TOTAL 0.00s; 1 passed, 1 failed, 36 not run.');
  });
