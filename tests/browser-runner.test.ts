...
describe('browser regression groups', () => {
  it('partitions group 2 into disjoint concurrent runner shards without omissions', async () => {
    ...
    expect(new Set(shards.flat()).size).toBe(35);
    expect(shards.map(shard => shard.length)).toEqual([9, 9, 9, 8]);
  ...
  it('partitions every existing browser entry exactly once and preserves full-suite order', async () => {
    const expected = [
      ...
      'cloud-backup-lifecycle', 'local-history-backup', 'history-restore', 'backup-admin-ui', 'admin-collection-ui',
  ...
  it('executes the full local suite sequentially and records every script and the total duration', async () => {
    ...
    expect(log.mock.calls.filter(([line]) => line.startsWith('[browser] PASS'))).toHaveLength(36);
    expect(log).toHaveBeenLastCalledWith('[browser] TOTAL 3.60s; 36 passed, 0 failed, 0 not run.');
  });
  ...
  it('stops a failing group, preserves its failure and explicitly reports scripts that did not run', async () => {
    ...
    expect(log).toHaveBeenLastCalledWith('[browser] TOTAL 0.00s; 1 passed, 1 failed, 33 not run.');
  });
