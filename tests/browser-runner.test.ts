...
describe('browser regression groups', () => {
  it('partitions every existing browser entry exactly once and preserves full-suite order', async () => {
    const expected = [
      ...
      'unread-counter', 'reaction-history', 'vault-lifecycle', 'voice-lifecycle', 'voice-submission',
  ...
  it('executes the full local suite sequentially and records every script and the total duration', async () => {
    ...
    expect(log.mock.calls.filter(([line]) => line.startsWith('[browser] PASS'))).toHaveLength(22);
    expect(log).toHaveBeenLastCalledWith('[browser] TOTAL 2.20s; 22 passed, 0 failed, 0 not run.');
  });
  ...
  it('stops a failing group, preserves its failure and explicitly reports scripts that did not run', async () => {
    ...
    expect(log).toHaveBeenLastCalledWith('[browser] TOTAL 0.00s; 1 passed, 1 failed, 19 not run.');
  });
