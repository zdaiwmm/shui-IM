...
describe('reply swipe resistance', () => {
  it('is monotonic, bounded and has visible resistance at the activation distance', () => {
    const samples = [0, 8, 24, 48, REPLY_SWIPE_THRESHOLD_PX, 360, 1000].map(replySwipeOffset);
    ...
    expect(samples[4]).toBe(REPLY_SWIPE_THRESHOLD_PX);
    expect(samples[4]).toBeLessThan(REPLY_SWIPE_MAX_OFFSET_PX);
    expect(samples[5]).toBe(360);
    expect(REPLY_SWIPE_MAX_OFFSET_PX).toBeGreaterThanOrEqual(460);
  });
