...
- 本提交仅调整发布预检可用下限为 `max(4 GiB, 2×最大冷归档 + 2 GiB)`；持续备份准入仍为 8 GiB。应用内 `release.json` 仍为 `2026.09.22.3`（ops 门禁变更，未新增用户可见版本说明），文档对账不触发第二次部署。
- 切换前使用率 80.45%（`DISK_HIGH`）。安全清理约释放 570 MiB（journal vacuum 约 168 MiB、已轮转旧 messages/kern 日志约 186 MiB、一层悬空镜像层）；之后约 79.0%／可用约 7.9 GiB。官方 `quiet-room-capacity` 清理几乎未释放空间。受保护备份与失败切换归档未删；未做 docker builder prune。
- 固定入口核对 `DEPLOY_VERIFIED`（`sha=3a9b6eb0a59cdb6740bfbd14cb0e5069a1db1cb3`，`url=https://ai.shui.click`）；批次 `20260922T231448Z`，发布目录 `/opt/quiet-room/git-releases/20260922T231448Z-3a9b6eb0a59c`。
- 独立 `READBACK_OK` 于 `2026-09-22T23:16:20.209Z`，耗时 2950ms；镜像 `sha256:abeeeede502111674efcd90df76a9bb2e0b267682191bef35c84624de055a1f1`，容器 `quiet-room-app:3a9b6eb0a59cdb6740bfbd14cb0e5069a1db1cb3` 健康；HTTPS／数据库／存储、公开与容器产物、公开 WebSocket 通过。备份容器运行且健康探针为 `none`；维护标记不存在，管理员开启，通话关闭，TURN 不存在。证据：`/Users/achilles/Documents/Codex/shui-IM/.git/quiet-room-readback/20260922T231617258Z-3a9b6eb0a59c-success.json`。
- iPhone / WeChat 未验证。本次知识库对账不再次部署、不改磁盘门禁、不删服务器数据。
...
- 独立 iPhone 实验已由用户确认系统名称实际变更；正式 UI 的 iPhone 键盘与原生验证流程仍未完成真机验收，发布确认时已披露，不把自动回归记作真机通过。开发树承载当前会话与独立回读证据，文档树仍在执行对账，两者保留。知识库提交不再次部署。当前生产已由上方 4 GiB 门禁发布接替。

## 容量治理发布与独立回读（2026-09-22 19:32）

- 精确应用提交 `52c582c646f1b3c760337dcf6f58a17927b2a92c` 已获用户确认；PR [#196](https://github.com/zdaiwmm/shui-IM/pull/196)，main 完整 CI [35721010459](https://github.com/zdaiwmm/shui-IM/actions/runs/35721010459) 通过，本地最终 85 文件／693 项测试通过。
- 固定发布入口 `DEPLOY_VERIFIED`；批次 `20260922T112930Z`，冷备份 `data-20260922T112930Z-52c582c646f1.tar.gz` 已校验；镜像 `sha256:bd2f7a1c201710c081cc7ed22aad7fcb581e311b328f2462638acca7f4d4a0d9`。独立 `READBACK_OK` 证据 `quiet-room-readback/20260922T113221167Z-52c582c646f1-success.json`，2740ms，HTTPS／公开与容器产物／WebSocket 均通过。
- 主机容量工具与发布 helper 按受控校验安装，原配置保存在 `/etc/quiet-room/pre-capacity-5da2d3f`。三个定时器启用；两个生产容器已确认 `json-file max-size=10m,max-file=3`。发布后可用约 9.6 GiB，实际使用率 74.4%，可用内存约 1.36 GiB。
- 当前保留：发布前快照最新 3 份并额外保护失败事件关联快照；当前及两个成功旧版本、全部容器引用、失败事件镜像受保护；连续备份与业务数据不进入自动清理。每 5 分钟本机检查、每日定向保留、每小时 Nginx 轮转、journal 300 MiB／7 天。外部通知未接通。
- 在线 builder prune 在此次小内存主机导致服务失联，已取消自动缓存清理；正常重启后恢复。不得再次将带过滤条件的 prune 视为低资源操作，不直接删除 Docker 层目录。
- 持续备份检查暴露历史附件索引缺失。用户另行确认后，保留完整冷快照硬链接、修复前数据库副本与原字段，只补齐一个附件的两条分块索引及大小计数，未改写附件文件。回滚目录 `/opt/quiet-room/backups/maintenance/attachment-index-20260922T113830Z`；持续备份 `quiet-room-2026-09-22T11-38-32-625Z` 已生成并校验通过。生成备份及修复副本后可用约 8.6 GiB（使用率 76.9%）；本机告警为容量偏高及异地复制缺失。未配置外部通知，不宣称已有异地灾备。
- 下方“未启用”策略是本轮实施前历史记录；当前事实以上述记录及容量治理需求为准。本次文档对账不再次发布应用。

## 多浏览器接入已发布与容量恢复（2026-09-22 18:24）

- 用户确认发布精确提交 `2b724dc721d4c4b810e0d506b172a60c52d14dff`，版本 `2026.09.22.2`；PR [#192](https://github.com/zdaiwmm/shui-IM/pull/192) 的两层授权功能和 PR [#194](https://github.com/zdaiwmm/shui-IM/pull/194) 的运行镜像依赖修复均已上线。通行密钥后由在线旧端认可新浏览器，再按空间申请对方批准；申请十分钟有效、同排入口可重开、到期清理提示。新设备无加入前历史，原设备继续可用。
- 精确 main CI [35699744004](https://github.com/zdaiwmm/shui-IM/actions/runs/35699744004) 完整通过；修复候选本地 687 项单元测试和 40/40 浏览器脚本通过。固定发布入口取得并核对 `DEPLOY_VERIFIED`；批次 `20260922T102241Z`，发布目录 `/opt/quiet-room/git-releases/20260922T102241Z-2b724dc721d4`，校验后的冷备份 `/opt/quiet-room/backups/predeploy/data-20260922T102241Z-2b724dc721d4.tar.gz`。镜像构建 19 秒，冷备份 49 秒，服务器部署 109 秒，入口总耗时 186918ms。
- 独立 `READBACK_OK` 证据 `quiet-room-readback/20260922T102451927Z-2b724dc721d4-success.json`，总耗时 3457ms；精确线上提交、镜像 `sha256:755e743acc7469677dd4433ada25d517bb7e1b22ef612e3d3b59515be08622aa`、公开产物、HTTPS 数据库/存储健康、新 WebSocket 均核对通过。应用健康，备份容器运行。真机 Safari/WebView、真实推送和独立安全审计仍未验证。
- 发布前已依用户逐项确认删除旧 saas 代码、PM2 应用、两个容器、独立数据库卷和专属镜像；删除 12 份旧 `data-*.tar.gz`，共 6,967,308,703 字节，清理时保留最新 3 份。连续备份、失败切换快照和当前数据均保留。恢复经服务器正常重启完成；Docker 单独重启未取得完成回执。曾有全量 Docker 统计期间的严重守护进程内存压力，后续停止全量扫描。
- 清理后发布前根分区可用约 8.2 GiB；本次发布后可用约 7.3 GiB、使用率 81%，内存可用 1394 MiB。新发布又产生一份冷备份；自动保留配置未修改，不能将手动保留 3 份记为已启用自动策略。
- 增长来源包括约 9.8 GiB 的原发布快照、累积历史镜像、约 2 GiB 日志和旧项目。实际在线备份配置为每 24 小时一次、14 份，容器启动也创建备份；应用日志为无轮转选项的 `json-file`。发布 helper 默认保留 14 份发布前快照和 5 个发布目录，不清理镜像/构建缓存。镜像共享层未作物理空间完整统计，不直接累加标称大小。
- 用户要求发布后输出保留策略；下方策略是待实施建议，本次未新增自动清理或监控，未调整生产 helper/Compose/系统日志配置。当前相关开发与发布工作树仍承载本会话证据，保留；本次文档对账不触发再次部署。

## 容量保留策略建议（2026-09-22，未启用）

| 对象 | 建议保留及触发规则 |
| --- | --- |
| 发布前快照 | 最新 3 份已校验快照，保护当前切换关联快照；预算 3 GiB，超额告警不强删最后恢复点 |
| 失败切换快照 | 未结案全部保留；结案、转存并校验后，本机保留 7 天，预算 1 GiB |
| 在线备份 | 异地备份和恢复验证完成后，本机保留最新 3 份（预算 3 GiB）；异地保留 14 个日、4 个周、3 个月恢复点。此前不缩减现有连续备份 |
| 应用镜像/发布目录 | 当前版本及最近 2 个成功版本，另保护容器引用、正在发布和未结案资源；按不可变 ID 定向清理 |
| 构建缓存 | 最近 7 天、目标上限 2 GiB，低负载与发布互斥清理 |
| 日志 | journal 300 MiB/7 天；容器每文件 10 MiB、3 份；Web 日志每天轮转、最多 7 份、20 MiB 触发提前轮转，日志合计目标 1 GiB |
| 业务数据 | 不进入磁盘自动清理；增长超过预算时迁移或扩容 |

- 目标磁盘使用率不高于 70%；75% 告警，80% 或可用不足 4 GiB 时阻止新的发布构建，90% 紧急处置。还须按新备份、构建峰值、失败归档与 2 GiB 应急余量计算单次发布空间；4 GiB 不是足够空间的保证。当前 81% 已超过建议门禁，应在下次发布前处理。
- 每 5 分钟轻量监控分区/inode/可用内存/备份新鲜度，避免全量 Docker 空间扫描；可用内存低于 400 MiB 持续 5 分钟告警，暂停额外构建。发布与独立回读成功后、每日低负载窗口定向清理，保护规则与容量目标冲突时先告警/迁移/扩容。
- 连续备份超过 36 小时或异地复制超过 24 小时未成功时告警；每月隔离恢复演练。在线备份数量不等于覆盖天数；异地备份须独立权限、版本化和删除保护。
- 先确认策略，再分步实施监控/发布门禁、轮转和定向保留；在线备份缩减必须满足异地前提。验收包括连续发布、失败保护、低容量拒绝、轮转实效、备份失败保留旧快照与发布互斥。当前没有承诺 40 GiB 可容纳无限业务增长。
- [Docker 官方说明](https://docs.docker.com/engine/logging/configure/)确认默认 json-file 无自动轮转；已有容器须重建才应用新日志选项。

## 多浏览器接入首次发布失败并回滚（2026-09-22）

- 用户确认发布主线精确提交 `9cf595c9f670cd8faebcbb87d16ad2c5ed2deded`（PR #192，包含此前已发布的 #191）；精确 main CI 35695610152 通过。固定入口构建镜像并校验切换前冷备份后，新应用容器无法通过健康检查。发布器验证失败切换归档，恢复原备份并返回 `ROLLBACK_OK`、`RELEASE_BLOCKED`，未取得 `DEPLOY_VERIFIED`。
- 失败源于 Docker 运行阶段未复制新服务端模块 `src/lib/browser-access-proof.mjs`。回滚后只读检查显示应用与备份容器运行、应用健康、维护标记不存在，服务器应用 SHA 和镜像为上一批 `013617cc81f96b79a8450543ea4c42471173a190`。独立回读在公开大文件下载时连接中断，故完整 `READBACK_OK` 尚未取得；脱敏失败证据 `quiet-room-readback/20260922T070551404Z-013617cc81f9-failure.json` 和二次回读 `quiet-room-readback/20260922T071402110Z-013617cc81f9-failure.json`。容器直接下载 CSS 完整，公开入口中断；服务器根分区已满（40 GB、可用 0），发布前备份约 9.8 GB。未删除任何备份或镜像，公开资源可用性仍待恢复和回读。
- 修复在独立工作树补齐运行镜像文件并增加回归检查。新的精确主线 SHA 需再次获得用户生产发布确认；本段仅记录失败和回滚，不触发部署。
...
- 冻结候选 `36d6adeae75d8fa7706f378b828e2a860394838a` 与发布提交文件树一致；本地 `check:full` 通过 81 个测试文件／665 项和 39/39 浏览器脚本。精确 main CI [35637008372](https://github.com/zdaiwmm/shui-IM/actions/runs/35637008372) 全部通过。
- 固定入口取得 `DEPLOY_OK`、`DEPLOY_VERIFIED` 并核对精确回执；批次 `20260921T181833Z`，发布目录 `/opt/quiet-room/git-releases/20260921T181833Z-5cc445d7be86`，切换前冷备份 `/opt/quiet-room/backups/predeploy/data-20260921T181833Z-5cc445d7be86.tar.gz` 已校验。镜像构建 51 秒，服务器部署 145 秒，固定入口总耗时 174169ms。
- 独立 `READBACK_OK` 于 `2026-09-21T18:21:30.855Z` 核对线上 SHA、镜像 `sha256:34f448fcd932079da02e5f567d3071ca9b8241c4d0cd7dc752b5931e3ffe52c8`、HTTPS／数据库／存储、公开产物及新建 WebSocket 一致。应用运行且健康，备份容器运行但无健康探针，维护标记不存在；管理员开启，通话关闭，TURN 不存在。脱敏证据 `quiet-room-readback/20260921T182127730Z-5cc445d7be86-success.json`，回读耗时 3124ms。
- iPhone 15 Pro / iOS 27 / Safari 真机尚未验证；独立审计等既有高安全门槛未完成。发布后在不含部署配置的独立文档工作树对账；文档提交不改变线上应用 SHA，不再次部署。
...
## 历史恢复批量读取与持久重试上线（2026-09-14 20:23）

- 用户明确要求将 PR [#155](https://github.com/zdaiwmm/shui-IM/pull/155) 合并并发布生产。修复提交 `21942b3534838a12be5400f0e1b9c95a81291524` 合并为线上应用 SHA `8f18cc19381a6878bd8fcc4cff732949c16aa632`，两者文件树一致；版本 `2026.09.14.6`。精确 main CI [34842403406](https://github.com/zdaiwmm/shui-IM/actions/runs/34842403406) 全部通过。本地构建、645 项测试与 34/34 浏览器脚本通过，Chrome／WebKit 另通过恢复专项。
- 修复合法表情被归档校验拒绝的问题；同归档每次最多读取 20 段／4 MiB，保留逐段认证；自动备份合并不足 100 条的短时增量。恢复核验比较本机差异，展示待恢复数量，应用删除投影后逐片提交。恢复任务和输入码仅保存在本机加密保险库，离页或锁定暂停后可以重试，完成或主动取消才清除；不进入云端检查点或恢复导出。输入弹窗移除粘贴与提示行，并适配短视口。
- 固定入口取得 `DEPLOY_OK`、`DEPLOY_VERIFIED` 并通过外层精确回执核验。批次 `20260914T122052Z`，发布目录 `/opt/quiet-room/git-releases/20260914T122052Z-8f18cc19381a`，切换前冷备份 `/opt/quiet-room/backups/predeploy/data-20260914T122052Z-8f18cc19381a.tar.gz` 已校验。
- 独立 `READBACK_OK` 于 `2026-09-14T12:23:21Z` 确认 SHA、镜像、HTTPS／数据库／存储、公开产物和新建 WebSocket 一致。应用运行且健康，备份容器运行但无健康探针，维护标记不存在。证据 `quiet-room-readback/20260914T122318543Z-8f18cc19381a-success.json`。
- 未验证实体 iPhone / iOS 27 / Safari 或用户真实恢复材料。旧码在换码后不能继续在线取件；旧归档仍需核验全部删除投影后才导入，不能承诺第一条下载完便显示；旧备份缺失的删除标记无法追溯。既有高安全发布门槛未因此完成。本次后续文档对账不触发再次发布。
...
<a id="release-20260914-0643"></a>

## 当前线上发布记录（2026-09-14 14:43）

- 用户确认发布精确 main 提交 `3e1f5cc4614cdbecf779361b25bdd62c554aa574`；PR #146 已合并，应用版本为 `2026.09.14.3`。精确 main CI [34813696088](https://github.com/zdaiwmm/shui-IM/actions/runs/34813696088) 全部通过。
- 固定发布入口返回 `DEPLOY_VERIFIED`；服务器批次为 `20260914T063849Z`，发布目录为 `/opt/quiet-room/git-releases/20260914T063849Z-3e1f5cc4614c`，切换前冷备份为 `/opt/quiet-room/backups/predeploy/data-20260914T063849Z-3e1f5cc4614c.tar.gz`，已完成校验。
- 独立只读回读于 `2026-09-14T06:41:35Z` 返回 `READBACK_OK`；线上 SHA、应用和备份容器、HTTPS 健康、数据库／存储、公开产物及公开 WebSocket 均匹配。证据保存在 `quiet-room-readback/20260914T064135274Z-3e1f5cc4614c-success.json`。
- 本次上线支持恢复历史内容时合并同一会话多个设备的有效 QR3 恢复码，聊天和保险箱归档自动去重，并在没有新增记录时显示明确原因。仍需每台设备的有效恢复码；服务端不保存恢复码或明文内容。实体 iPhone / iOS 27 / Safari、生产管理员逐项操作、OSS 异地备份、外部监控告警、独立密码学审计和高安全发布门槛仍未完成。

<a id="release-20260914-0431"></a>

## 当前线上发布记录（2026-09-14 12:31）
...
## 当前线上发布记录（2026-09-14 10:07）

- 用户确认发布精确 main 提交 `ad47f0faa9faa7459bbfe042afcead846d0128b2`；PR #140 已合并，应用版本为 `2026.09.14.1`。精确 main CI [34797058185](https://github.com/zdaiwmm/shui-IM/actions/runs/34797058185) 全部通过。
- 固定发布入口首次到达生产部署阶段时被已有部署互斥锁拦截；随后只读检查确认目标 SHA 已完成切换，重试被安全地拒绝为“已经部署”，没有重复切换。服务器批次为 `20260914T020142Z`，发布目录为 `/opt/quiet-room/git-releases/20260914T020142Z-ad47f0faa9fa`。
- 独立只读回读于 `2026-09-14T02:07:07Z` 返回 `READBACK_OK`；线上 SHA、应用和备份容器、HTTPS 健康、数据库／存储、公开产物及公开 WebSocket 均匹配。成功证据保存在回读工作树 Git 元数据的 `quiet-room-readback/20260914T020703855Z-ad47f0faa9fa-success.json`。
- 本次版本修复小屏幕下备份与恢复页面按钮裁切、待发送消息隐藏提示、通话不可用原因和表单焦点边框显示。保险箱入口辨认度与横竖屏限制保持未变；实体 iPhone / iOS 27 / Safari、生产管理员逐项操作、OSS 异地备份、外部监控告警、独立密码学审计和高安全发布门槛仍未完成。后续文档提交不代表再次部署。

<a id="release-20260913-2314"></a>

## 当前线上发布记录（2026-09-13 23:14）

- 用户确认发布精确 main 提交 `8f946e78a6603155f221a96eb085d15510ddd1cf`；集成 PR #129、#131、#132 及版本元数据 PR #133 已合并，应用版本为 `2026.09.13.1`。
- 精确 main CI [34764358330](https://github.com/zdaiwmm/shui-IM/actions/runs/34764358330) 全部 12 项通过；集成工作树 `npm run check:full` 通过（78 个测试文件、638 个测试，浏览器套件 33/33）。
- 固定入口对该 SHA 返回 `DEPLOY_OK` 与 `DEPLOY_VERIFIED`；服务器批次为 `20260913T151432Z`，发布目录为 `/opt/quiet-room/git-releases/20260913T151432Z-8f946e78a660`，切换前冷备份为 `/opt/quiet-room/backups/predeploy/data-20260913T151432Z-8f946e78a660.tar.gz`，已完成校验。
- 独立只读回读于 `2026-09-13T15:17:10Z` 返回 `READBACK_OK`；线上 SHA、应用和备份容器、HTTPS 健康、数据库／存储、公开产物及公开 WebSocket 均匹配。成功证据保存在调用工作树 Git 元数据的 `quiet-room-readback/20260913T151710632Z-8f946e78a660-success.json`。
- 本次上线包含聊天媒体发送后的默认模糊与点击显露、表情发送占位稳定，以及一次性设备修复邀请和六位安全码核对后的故障设备替换流程；既有加密、设备历史边界和附件完整性边界保持不变。实体 iPhone / iOS 27 / Safari、生产管理员逐项操作、OSS 异地备份、外部监控告警、独立密码学审计和高安全发布门槛仍未完成；后续文档提交不代表再次部署。


<a id="release-20260913-2"></a>

## 当前线上发布记录（2026-09-13 21:12）

- 用户确认发布精确 main 提交 `232af20cd130ad9008faea063401d5f462d0cc23`；PR #127、精确 main CI `34751890061` 全部通过。
- 本批仅更新流程入口与发布对账文档，不包含新的用户可见产品代码变化；应用版本元数据仍为 `2026.09.12.3`，未制造空版本。固定入口返回 `DEPLOY_OK` 与 `DEPLOY_VERIFIED`。
- 服务器批次为 `20260913T130735Z`，发布目录为 `/opt/quiet-room/git-releases/20260913T130735Z-232af20cd130`，切换前冷备份为 `/opt/quiet-room/backups/predeploy/data-20260913T130735Z-232af20cd130.tar.gz`，已完成校验。
- 独立只读回读于 `2026-09-13T13:11:45Z` 返回 `READBACK_OK`；线上 SHA、应用/备份容器、HTTPS 健康、公开产物和公开 WebSocket 均匹配。回读证据保存在发布副本 Git 元数据的 `quiet-room-readback/`。
- 应用健康，备份容器运行中且无健康探针，记录为 `none`；后台开关 `1`，通话覆盖 `0`，TURN 容器不存在。实体 iPhone / iOS 27 / Safari、生产管理员逐项操作、OSS 异地备份、外部监控告警、独立密码学审计和高安全发布门槛仍未完成。

<a id="release-20260913-1"></a>

## 当前线上发布记录（2026-09-13 17:42）

- 用户确认发布精确 main 提交 `5bc057dad24a6043ac4d36757c2e99029e6c042a`；PR #125、精确 main CI `34732455717` 全部通过。
- 本批仅更新 Alibaba OIDC/SAS doctor 工作流的 CLI 参数，不包含用户可见产品代码变更；固定入口返回 `DEPLOY_OK` 与 `DEPLOY_VERIFIED`。
- 服务器批次为 `20260913T094232Z`，发布目录为 `/opt/quiet-room/git-releases/20260913T094232Z-5bc057dad24a`，切换前冷备份为 `/opt/quiet-room/backups/predeploy/data-20260913T094232Z-5bc057dad24a.tar.gz`，已完成校验。
- 独立只读回读于 `2026-09-13T09:45:04Z` 返回 `READBACK_OK`；线上 SHA、应用/备份容器、HTTPS 健康、公开产物和公开 WebSocket 均匹配。证据保存在调用工作树 Git 元数据的 `quiet-room-readback/`。
- 生产应用版本元数据仍为上一用户可见版本；实体 iPhone / iOS 27 / Safari、生产管理员逐项操作、OSS 异地备份、外部监控告警、独立密码学审计和高安全发布门槛仍未完成。
