...
The creator gallery entry is labeled 保险箱. Its single header row contains back, a 44px-high 图片 / 文件 segmented control with muted loaded-item counts, an eye visibility toggle, and upload. Opening it defaults to 图片 with all thumbnails blurred. Tap once to reveal a thumbnail and again to open the viewer; the eye / slashed-eye action shows or hides all current thumbnails. Thumbnails have no reveal text or time badge; the enlarged image details show its message/upload time. Visibility lasts only for the current visit, resets after leaving or locking, and does not change authentication or encryption. Newly arriving images start hidden. The file list appears only in 文件. Each tab has its own empty state and earlier-history loading; incomplete counts are distinguished from totals. Upload completion opens the category containing the newly uploaded item.
...
The pinned recovery reminder can be dismissed; that choice is encrypted locally. Preparing recovery shows its code immediately, then offers an explicit package download. Only confirming that both parts are stored separately records completed setup.
...
The package does not contain the encrypted IndexedDB history store. For MLS rooms, recovery uses its identity key to sign a short-lived replacement request binding a fresh device identity and access token. Another active trusted device must be online to validate that proof and commit one MLS Remove+Add replacement. All other active devices must have opened a version supporting `recovery-replace-v1`. The original device is fenced while a request is pending, then permanently revoked when replacement succeeds. Requests expire after at most 15 minutes, releasing the original device if recovery has not completed.

The restored device joins the new epoch at the replacement's message and receipt boundary. It cannot fetch or decrypt earlier history and never reuses the old sending ratchet, even if the original device sent messages after the package was saved. Save a new package and code after recovery; the old package cannot authorize another replacement of the revoked original device. Legacy non-MLS rooms retain their existing import path and do not gain MLS forward secrecy.
