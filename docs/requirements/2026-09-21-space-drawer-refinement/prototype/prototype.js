...
function drawer(){const active=rooms[current];return `<div class="overlay" data-action="chat"></div><section class="drawer space-index" role="dialog" aria-modal="true" aria-label="私密空间">${header('私密空间')}<div class="rows"><button class="current-space ${scene==='menu'&&edit===current?'context':''}" data-room="${current}" aria-current="true" aria-label="${escape(active.name)}"><span class="current-eyebrow">当前空间</span><span class="current-name"><strong>${escape(active.name)}</strong><span class="current-tick">${icon('check')}</span></span>${active.pending?'<small class="current-pending">等待对方加入</small>':''}</button><div class="other-heading"><span>切换空间</span><span>${rooms.length-1}</span></div><div class="other-spaces">${rooms.map((r,i)=>i===current?'':row(r,i)).join('')}</div></div><footer class="drawer-foot"><button class="create" data-action="create">${icon('plus')}<span>创建新空间</span></button><button class="settings-entry" data-action="settings" aria-label="设置">${icon('gear')}</button></footer></section>`;}
