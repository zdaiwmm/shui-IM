...
await page.getByRole('button',{name:'修改名称',exact:true}).waitFor();
await page.getByRole('button',{name:'修改名称',exact:true}).click();
if(!await page.locator('.pk-name-dialog input').evaluate(el=>document.activeElement===el))throw Error('Name input was not focused');
await page.getByRole('button',{name:'取消',exact:true}).click();
await page.getByRole('heading',{name:'Quiet Room · 日常空间'}).waitFor();
await page.goto(`${base}?screen=1`);
await page.getByRole('button',{name:'创建私密空间',exact:true}).click();
await page.getByRole('button',{name:'创建通行密钥并创建空间',exact:true}).click();
await page.getByRole('heading',{name:'空间已创建',exact:true}).waitFor();
if(await page.getByRole('button',{name:'申请对方授权',exact:true}).count())throw Error('Creator incorrectly routed to join authorization');
await fs.writeFile(path.join(output,'render-check.json'),JSON.stringify({pageErrors:failures,dimensions,prototypeCancelPath:'passed',nameInputFocus:'passed',creationToInvitation:'passed',directSettingsVerification:'passed',note:'Static/prototype rendering only. System verification not simulated as real authentication.'},null,2));
await browser.close();
