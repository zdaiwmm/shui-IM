...
- The gallery has separate 图片 and 文件 tabs and defaults to 图片 on entry. Images retain their thumbnail grid and viewer; files use a download list. Tabs support arrow/Home/End keys and load earlier local history for the selected category. Completing an upload shows its corresponding category.
...
- Chat uses native document scrolling so real message content can extend behind Safari chrome. The header and composer stay attached to the visual viewport, with safe-area padding and keyboard space. Each floating control samples the conversation through a bounded glass filter, soft vertical sheen and a single clean border; the edge wash has no additional blur. Reduced transparency/high contrast uses opaque controls. Pages enter without translating their layout; viewer, menu, and local-security notices honor `prefers-reduced-motion`.
...
- The centered status capsule combines “我” and its status icon before “对方” and its status icon on the first line. The second line shows the peer’s “在线” state or time since last online. Self presence remains accessible by its label and status icon without a separate left-hand control. The last-online timestamp stays in server memory and is unavailable after a server restart.
