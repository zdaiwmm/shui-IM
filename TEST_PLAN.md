...
13. 点按已显露的单图或相册格从正确索引打开 overlay；前后按钮、触摸横划、键盘方向键、计数、当前原图下载和关闭焦点恢复一致。图片层无装饰内边距，按比例播满视口宽度，仅在高度先达到视口边界时允许水平留白；查看器背景须使用当前日间／夜间页面背景且不得添加图片高斯模糊层。正常缩放范围为默认适配尺寸的 1/3×–5×；越界手势有阻尼并在松手后回弹。下拉关闭过程随距离增加阻尼并逐渐缩小图片。横向分页必须跟手且有阻尼，松手后使用双层 380ms 平滑交接，顶部标题不重建或闪烁；双击放大／复位经过逐帧 transform 动画。从图片切到视频时视频默认暂停，仍可横划到后续图片；离开视频立即暂停并卸载 source。切页不得重排聊天或产生新消息。聊天向下拖拽或查看器下拉关闭后隐藏全部聊天缩略图；隐私遮盖层同步清除显露状态并关闭查看器，包括移动端短暂失焦后快速恢复、缓存重建和迟到解码。拖拽后的合成点击不得再次显露。
