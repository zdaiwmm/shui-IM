...
- 2026-09-05 01:02（Asia/Shanghai），最终应用工作树 `npm run check:full` **全程通过**：构建、43 个文件／311 项单元集成、21／21 浏览器入口，浏览器汇总 163.92 秒。`npm run test:calls:e2e` 两项专项通过；最终 WebKit 生命周期、回到底部和时间线专项均通过。两引擎共 50 张合成截图、各 24 种布局均完成；新时间线浏览器错误为 0，末条间距 63.6875–64.375px，关键明暗／大字号／极端图片／历史按钮已目检。
...
- 候选为 `codex/message-timeline-polish`，基于 `93c93479c9384e7dde862e9bf97e308eb2ad47c3` 的未提交工作树；上述验证不等于 GitHub CI、合并或生产发布。后续证据见[状态页](./docs/context/status.md)。Playwright WebKit、模拟键盘和合成媒体不等于真实 iPhone Safari 验收。
