...
5. 每次 `blur` 与 `visibilitychange(hidden)` 在同一事件调用栈内显示常驻不透明遮盖层，不等待下一帧；`hidden`、`pagehide`、`freeze`、BFCache 恢复、手动/空闲锁定及系统组件失焦立即清理会话、WebSocket、对象 URL 和传输。普通 `blur` 只将会话清理防抖 250 ms，快速恢复可取消待定清理。仅会话和 socket 打开前的网关 WebAuthn 有有界豁免。
...
- 旧版本手势的一次性正确/错误迁移路径；
- 普通短暂 `blur` 防抖、持续失焦锁定；系统组件失焦立即白屏、前台不自动恢复；
