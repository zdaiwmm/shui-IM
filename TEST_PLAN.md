...
| 设备绑定保险库 | 随机主密钥；Argon2id 手势与 WebAuthn PRF 共同派生包装密钥；拒绝可同步凭据；恢复后换新凭据 | 真实 Chrome 虚拟 CTAP2.1/PRF 验证、新建/解锁/旧库迁移/恢复重绑定 | 实机浏览器矩阵和硬件密钥兜底验证 |
...
4. 连续失败从第 4 次起指数退避，最长 30 秒；正常保险库同时需要不可同步设备凭据，恢复包需要独立随机 256 位恢复码。
5. `visibilitychange(hidden)`、`window.blur`、`pagehide`、手动锁定和空闲锁定复用幂等锁定入口，立即显示纯白隐私帘并清理会话引用、WebSocket、对象 URL、队列视图和文件传输；仅用户主动打开聊天/相册图片选择器期间豁免选择器自身造成的失焦或隐藏。
