...
6. 回到前台不得自动恢复；右下角长按需显示克制的按住进度提示，达阈值后直接发起通行密钥验证，不播放完成扩散环／烟花，不等待或自动点击中间页按钮。取消只保留中性重试入口，不显示红色错误文案；真实失败可手动重试。v3 网关原生请求在锁定、pagehide、freeze 和 65 秒到期后须收到 AbortSignal，重复进入不残留 pending 请求；真实 Safari 的原生弹窗和面容流程须另外真机验证。
