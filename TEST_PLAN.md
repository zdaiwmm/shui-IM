...
## 表情最近发布、合集排序与相册补位（2026-09-10）

- `tests/meme-picker.e2e.mjs` 覆盖默认 GIFs 的“最近发布”标题、精确 10 项、后续结果不重复及五列布局；覆盖三个已添加合集的 500ms 长按浮起、插入末位、原位松手回归、加密顺序持久化和 Alt+左右方向键排序。
- `tests/file-interactions.e2e.mjs` 删除真实本机投影的聊天图片后，检查相册网格 DOM 保持挂载、其余缩略图产生补位位移、精确数量与滚动容器连续，同时确认底层聊天消息及其顺序不变。
- 本地候选 `npm run check:full` 从头通过：78 个 Vitest 文件／635 项单元与集成测试、31／31 个顺序 Chromium 浏览器入口，浏览器汇总 420.61 秒；文档检查和差异格式检查通过。390px“最近发布”截图已目检。桌面真实浏览器自动化验证 DOM、持久化和动画触发，不替代 iPhone / iOS 27 / Safari 对长按、跟手拖拽、滚动和删除补位手感的真机验收。


## 聊天反馈与已读（2026-09-10，任务分支，暂不合并）

- 爱心：在线合心更强；双方在线的本机发送/对端实时消息分别从左右发电流，抵达后整心抖动，空闲双拍增强；历史同步不触发，合心期间排队，离线、隐私与减少动态效果取消。`presence-circuit.e2e.mjs` 覆盖 Chrome/WebKit、明暗主题和尺寸。
- 输入提示为浅灰色；空输入需持续按住 500ms 才开始录音，300ms 普通鼠标点击和触屏短按不申请麦克风。`voice-gestures.e2e.mjs` 继续覆盖长按发送、取消、失效权限和布局。
- 单勾为已发送，接收成功仍单勾；双勾只来自对方前台显示的有效加密已读事件。文字、语音、文件卡片可见即读，图片/视频需完成校验、加载并显露；语音已读不代表已听完。
- `message-read.test.ts` 检查身份/目标/事件顺序/媒体种类/重放；`message-read.e2e.mjs` 检查双勾、DOM 保留、加密待发与历史、前台/能力门；`browser.e2e.mjs` 检查真实 MLS 双端读回执。已读事件不新增气泡或未读数，不改变接收协议。
- 最终验证为 `npm run check:full`；真机 iPhone / iOS 27 / Safari 单独验收。

## 媒体已读隐藏与右拖（2026-09-10）

- 发出的单图、相册、视频和表情未读持续显露；已送达、同角色关联设备、错误目标或未确认的事件不得启动隐藏计时。对方在聊天可视区显露并完成校验后才产生加密已读事件；隐藏、离开聊天、失焦和旧设备能力不足时不得发送。
- 首次有效已读事件获服务器接受 10 秒后隐藏；重放不延时，迟到事件和重新打开历史沿用原截止时间。隐藏后手动显露可用，隐私清理即时隐藏并销毁计时器。消息、原图和已加密历史不删除。
- 媒体向右拖动具有递增阻力，达到阈值松手隐藏全部预览并平滑回位；短拖、取消、普通纵向滚动和左拖回复不得误触发。视频共用该手势，查看器下拉关闭沿用原规则。
- `media-read.test.ts` 覆盖目标、角色、重放与输入校验；`crypto.test.ts`、`mls.test.ts` 覆盖加密收发；`chat-image-privacy.e2e.mjs` 覆盖已读期限、持久加密待发箱、显露和隐私清理；`browser.e2e.mjs` 覆盖真实双端 MLS/WebSocket 读后计时。表情专项检查聊天图片四角 12px 与顶部快捷图 6px，视频专项检查右拖。
- 自动浏览器证据与 iPhone / iOS 27 / Safari 真机证据分别记录，自动测试不替代真机手感验收。
...
- `tests/attachment-favorites.test.ts`：收藏精确标识、损坏/稀疏/重复数据拒绝、手动过滤和置顶顺序。附件收藏与独立梗图副本的删除语义分别回归。
- `tests/voice-gestures.e2e.mjs`：空输入短按输入、350ms 长按录音、有字长按保持原生编辑、松手单次发送、闭合弧线按实际路径命中、滑出恢复、红色取消、触摸中断、权限迟到与锁定停止；320/390/768/1280px 检查两端留白和区域几何。
- 所需门禁为 `npm run check:full`，通话另跑 `npm run test:calls:e2e`。浏览器模拟权限和音轨不代表真机；iPhone / iOS 27 / Safari 仍需验证系统相册多选、拍摄照片/视频模式、键盘发送、真实麦克风授权及圆弧滑动手感。
- 本节替代下方历史记录中独立麦克风、文本发送按钮、标题栏通话及独立保险箱入口的旧验收；现有梗图按 [MEMES.md](./MEMES.md) 保留。

## 2026-09-08 保险箱图片属性与动图

- 后续回归：滚动条隐藏仍可原生滚动、鼠标／触摸上拉全屏、全屏焦点限制；明暗主题切换与开关查看器均保持透明 theme-color 提示，照片顶底不叠加额外色带。长按菜单必须先把焦点交给已连接查看器再移除，迟到 contextmenu 不重开菜单。手机复测偶发进入遮蔽层；自动化验证 DOM 交接与原有离开即遮蔽边界，不证明已重现该偶发真机现象。Safari 系统栏由浏览器控制，不承诺强制透明。
- 视觉反馈回归：保险箱查看器无下载入口，聊天原有下载保留；信息按钮位于顶栏，详情沿用玻璃变量。专项覆盖外围点击、鼠标/真实触摸下拉、内容滚动、520ms 连续下行动画、明暗窄屏截图，以及进入保险箱时退场聊天图片、时间和回执逐帧不可见，相册在整个过渡中保持满屏高度。真机仍需复核玻璃观感、手势手感和曾报告的闪图。
- 自动专项：`npx vitest run tests/image-photo.test.ts` 覆盖 GIF、WebP、APNG 原始字节加密往返、静态与损坏容器、取消、EXIF 属性、日期来源和 GPS 边界。`node tests/photo-details.e2e.mjs` 已接入 `check:full`，覆盖保险箱长按入口、聊天无属性入口、真实 EXIF 的文本渲染、明暗/横竖屏布局、实际动图像素变化与静态暂停、重新打开和翻页、对象 URL 回收及迟到解析/锁定清理。
- 人工验收：真实 iPhone / iOS 27 / Safari 从系统选择器分别导入 GIF、动态 WebP、APNG，验证自动播放、静态开关、双指缩放、属性面板滚动和后台锁定；用含/不含拍摄时间及 GPS 的原图核对显示来源。浏览器或系统选择器移除的属性不可恢复，不把文件修改时间标成拍摄时间。Live Photo 本次明确排除。
- 自动化、真机、CI、合并及生产状态见 `docs/context/status.md` 的本任务记录；本地模拟不能代替真机验收。

## 2026-09-06 发布后线上反馈候选

- 候选从已对账的 `origin/main` `9ea07b1eda3bcc21de4b29b4c355f8877b00abe2` 建立，覆盖静默焦点恢复、回复拖拽上限、图片缩小回弹、视频详情尺寸、相册翻页连续性、媒体密文缓存、语音状态文案和消息时间颜色。
- 冻结工作树 `npm run check:full` 从头通过：构建、52 个 Vitest 文件／422 项单元集成测试，以及 23／23 个顺序 Chromium 浏览器入口，浏览器汇总 257.91 秒。文档检查与差异格式检查通过；真实 iPhone iOS 27 Safari、CI、合并和生产发布仍需单独取证。

## 2026-09-06 下午集中候选

- 从最新 `origin/main` `b91a3a36fc7cb61c467347798f61c1f79e8acf39` 建立独立集成工作树，逐一核对并纳入通行密钥回焦、媒体准备反馈、图片查看器、左滑回复、语音滑动反馈、任务隔离规则和结构化生产回读；已通过 patch 等价检查排除早先已由 PR #26 合并的重复遮蔽激活提交。
- 并行文档的 D-027 编号冲突已拆为媒体准备 D-027 和语音反馈 D-028。回复手势改为 window 级松手监听后，消息删除竞态夹具同步改为向真实全局监听发送松手事件，继续证明目标删除后不能被迟到手势重新引用；定向回归通过。
- 最终组合 `npm run check:full` 从头通过构建、52 个测试文件／421 项单元集成和 23／23 个顺序 Chromium 浏览器入口，浏览器汇总 254.96 秒；文档检查与差异格式检查通过。自动化与旧版真机取证不能替代修复后真实 iPhone Safari 验收；CI、合并与生产须继续单独取证。

## 遮蔽入口与通行密钥遗留请求复查（2026-09-06）

- 本次候选移除完成长按的扩散环／烟花，保留按住提示，不调整一秒门槛或系统验证期限。
- `desktop-privacy.e2e.mjs` 保留真实 gateway、`withDeviceVerification` 和 platform-vault
  调用链，仅替换原生凭据接口。修复前复现锁定后原生请求仍 pending；修复后锁定、pagehide、
  freeze、65 秒到期均收到 AbortSignal，慢请求不并行重试，重复进入不留下旧请求。
  Chromium 由 CDP 投递可信触摸事件；Playwright WebKit 使用鼠标事件，二者均不是 iPhone
  原生验证弹窗的验收。旧测试的直接调用与迟到结果断言仍保留。
- 冻结候选 `npm run check:full` 从头通过：51 文件／413 项单元集成，23／23 浏览器入口，
  浏览器汇总 252.15 秒。WebKit 隐私专项、文档检查和差异格式检查通过。
- 真机加载的 `/assets/app-CIOXv7kU.js` 与旧发布一致，用户仍报告延迟／不弹框；撤回之前
  将异步存储等待认作完整根因的结论。新录屏另显示停留遮蔽页，不能仅靠图像区分长按被取消
  或松手时机。候选未合并／发布，不把桌面测试通过写成已解决全部真机症状。
- 后续用户确认未松手，且后台恢复易发、杀进程可恢复；使用本机临时书签取证，先区分事件取消
...
- 最终焦点修复候选 `266b714bd55f9c99d12085fcc69fe5adc817f233` 的 `npm run check:full`
  从头通过：51 文件／413 项单元集成、23／23 浏览器入口，浏览器汇总 253.41 秒；
  WebKit 隐私／系统表面专项通过。普通点按恢复焦点后正常弹框是用户对旧版的真机对照，
  不能记成新候选真机验收。新 CI、合并、发布均尚未完成。

## 聊天媒体准备反馈与默认遮蔽（2026-09-06，本地候选）

- 缺陷基线来自用户提供的 iPhone 截图：进入聊天后，部分照片／视频停留在无状态的静态媒体图标，点按没有可见反馈。根因为 `pending/loading` 点按被错误记为“已显露”，但未验证媒体尚无可呈现像素；异步完成后还可能跳过默认模糊状态。
- 冷加载卡片现在显示下载、逐分块解密和已验证预览生成阶段；可计算阶段显示百分比与进度条，等待／解码使用有界的非确定进度，减少动态效果时保持静态。准备中点按会主动开始或继续加载，但不改变显露状态；完整分块认证、原长度和 SHA-256 校验及本机解码完成后，才替换为默认模糊缩略图。失败保留原因和点按重试。
- `tests/chat-image-privacy.e2e.mjs` 新增慢下载夹具，验证准备中点按不显露、可见与无障碍 busy 状态、下载／解密／模糊预览三阶段反馈、完成后仍默认隐藏、随后首次显露和再次打开，以及原字节、锁定、迟到读取和缓存清理边界。Chromium 与 WebKit 定向专项通过，加载卡片及默认模糊截图已目检；视频专项通过。
- 2026-09-06 15:25（Asia/Shanghai），隔离候选从 `b91a3a36fc7cb61c467347798f61c1f79e8acf39` 建立，`npm run check:full` 从头通过：构建、51 个 Vitest 文件／413 项单元集成与 23／23 个顺序 Chromium 浏览器入口，浏览器汇总 251.04 秒。未运行 CI，未合并或部署；Playwright WebKit、移动 UA 和合成媒体不等于修复后的真实 iPhone Safari 验收。

## 真机录屏键盘白屏与输入区材质回归（2026-09-05）

- 缺陷基线来自用户提供的 iPhone 录屏：聊天已在底部时点按输入框，键盘展开阶段整段应用内容被纯白隐私帘覆盖，返回后标题与输入栏位置跳变；键盘收起阶段又会在中间视口提前显现输入工具栏。录屏证明旧版缺陷，不作为修复后的真机通过证据。
- 生命周期专项必须用真实受信 pointer 激活 textarea，再模拟 Safari 保持文档 visible 的一次 `window.blur`：1.2 秒一次性 keyboard handoff 内不得出现 `privacy-obscured` 或替换聊天 DOM；点按后的新键盘几何证据与第一次归属 blur 可任意顺序共同结算，窗口 focus 返回也可单独结算，只有 resize 证据时不得提前丢失 owner。程序化 focus、已打开键盘、第二次 blur、已消费交接的双时钟到期、hidden、pagehide、freeze、persisted pageshow、显式锁、空闲到期、离开聊天及输入框／session／runtime 变化仍须同步遮挡并锁定；未发生 blur 的闲置 token 到期只清理，不凭空锁定；选择器／媒体只能替代未消费 token，不能覆盖已消费 owner。不得放宽现有选择器和媒体授权交接测试。
- iPhone 固定定位原点回归须独立模拟原生原点与延迟的 `visualViewport.offsetTop`：覆盖换行期间 21／23 px 偏移、发送期间 48／49 px 偏移，以及失焦后原点先归零而视口仍停留在键盘端点。无 resize／scroll 事件的下一渲染帧标题须保持顶边、输入栏布局底边须保持可视高度；14 px 显现动画不得被写入持久定位。触屏 iOS 输入事件须立即完成真实 textarea 增高，收缩在下一帧读取最新输入，中文删除／提交的连续事件不得造成临时收缩，保留中文组合输入选区与焦点；后续帧不得继续插值编辑区域高度，消息内容仍平滑位移，历史阅读锚点不变。还须覆盖文档 padding 收缩使原生滚动及固定原点先行钳制：读取贴底目标前必须校正输入栏，不得二次补偿同一位移；静止 textarea 的 composer 祖先不保留 transform 或 will-change: transform。标题须使用无脚本平移的原生 sticky，固定原点探针独立于标题；还须模拟原生固定底边增加 40 px、clientHeight 不变，输入栏仍贴可视底边。键盘打开且贴底时连续多行输入和多次发送不得滚动原生文档，返回历史滚动时须恢复同一消息像素位置。桌面 WebKit 的合成坐标夹具不等于 iPhone 真机验收。
- 视口专项必须用与生产阈值一致的关闭／中间／打开三态，覆盖 resize 先于 focus／blur，以及打开和关闭各有多个超过 160 ms 的中间平台：期间 composer 保持完全隐藏，header 每个合并后的渲染帧贴可视区顶部且无 opacity/position 动画，至少一条真实消息与聊天可视区相交；list 的 `padding-bottom`／`min-height` 不写入，向下按钮不测量，也不强制滚动。只有目标 open/closed 状态稳定 160 ms 后才在一个端点批次内提交列表几何与贴底，并保持按钮测量有界，再从最终底边显现；1 条、3 条和长历史均重复该断言。完全没有目标几何的硬件／浮动键盘须在 640 ms 有界恢复；已开始移动的中间态不得使用该短回退；原生过渡以整段 1.6 秒为上限，包含低于方向阈值的中间态连续抖动，手指按住或手动滚动惯性仍等待释放和静止。首次挂载在中间态须隐藏；same-DOM 离开期间的时间和输入事件不得让返回首帧提前显现，恢复须从新样本重新计时。changed-frame 硬回退的布局和按钮更新也只能结算一次。手动滚动在 motion 期间不得分页、保存锚点、标记已读或测量按钮，稳定端点后这些账务各结算一次。
- `chat-bottom-control.e2e.mjs` 在浅／深色的闲置、textarea 聚焦和照片／语音禁用态逐一比较输入栈、照片、空闲语音和向下按钮的计算背景：四者分别共享 84%／92% 表面透明度与同一高光；可见控件元素 opacity 保持 1，向下按钮仅在其导航状态隐藏时用 opacity 0。降低透明度、高对比度、forced-colors 与无背景模糊环境使用不透明回退；深色高对比前景和 hover 强边界另行断言。Playwright 不能原生模拟的 reduced-transparency 与 no-backdrop 场景只作为真实 CSS 条件规则和级联的 source-rule fixture，不外推为系统偏好真机通过。
- 2026-09-06 10:11（Asia/Shanghai）前，最终本地候选重新运行 `npm run check:full` 从头通过：构建、51 个 Vitest 文件／413 项单元集成与 23／23 个顺序 Chromium 浏览器入口，浏览器汇总 251.43 秒；`npm run test:calls:e2e` 两项通过。最终代码的前端生命周期、回到底部和系统表面专项另在 WebKit 通过。覆盖初始中间态、分离键盘抖动硬上限、挂起恢复首帧、changed-frame 端点单次测量、离开聊天后的分页 continuation 和授权失败前停止麦克风请求；旧测试夹具已按真实异步边界等待，产品位置与安全断言保持有效。
- 最终代码由 [PR #24](https://github.com/zdaiwmm/shui-IM/pull/24) 合并为 `f7c2f1daa41c00a107685750d4be62cc892c15f2`；[PR 完整 CI](https://github.com/zdaiwmm/shui-IM/actions/runs/34005889972) 和[精确 main 完整 CI](https://github.com/zdaiwmm/shui-IM/actions/runs/34006132442) 均通过。2026-09-06 10:32:18 前取得固定入口精确发布回执及独立 `READBACK_OK`，首次停止检查失败后的回滚核验和有界重试见 [RELEASING.md](./RELEASING.md#本次线上发布记录)。桌面合成视口与用户的缺陷录屏都不能替代修复后真实 iPhone Safari／Chrome、安装态 PWA 和 Android Chrome 验收。

## 回到底部、语音删除与桌面验证跟进（2026-09-05）

- 回到底部动画冻结一次目标位置，动画帧不再重复读取最新消息和输入栏布局；程序化滚动期间跳过常规滚动测量，完成后统一校准、标记已读并恢复输入焦点。移动浏览器即使在按钮点击后强制触发一次 `blur`，仍须保持键盘意图和原选区；动画全过程标题栏固定，输入栏不进入键盘收起动效。
- `chat-bottom-control.e2e.mjs` 在 Chromium 与 WebKit 均通过：140/140.75px 短程约 367/369ms、2200/2200.75px 长程约 767/767ms，分别采样 22 与 46 帧，目标布局均只读取 3 次；焦点、选区、固定标题、取消、分页、异步媒体和 5000 条历史有界读取全部通过。
- 双端主流程实际长按已发送语音播放器，验证同一消息删除菜单可用、长按结束的合成点击不会误播放、执行“仅为我删除”后对应播放器卸载且数量减少。桌面 F 长按在手动锁定和刷新后直接启动新一轮真实设备验证，不再要求额外点击网关按钮；保留会话恢复仍不重复验证。
- 最终冻结工作树 `npm run check:full` 从头通过构建、50 个测试文件／400 项单元集成和 23／23 个浏览器入口，浏览器汇总 200.55 秒；`npm run test:calls:e2e` 两项通过，WebKit 底部定位与前端生命周期专项通过。自动化使用桌面浏览器和合成媒体，不替代真实 iPhone/Android 键盘、帧率、触感和权限验收；CI、合并与生产状态须分别取证。
...
- PR 首轮 Linux CI 的 Chromium 字体栈使元信息基线比最后字形框低约一个行框，旧测试以固定 5px 余量误报“增加两行”；气泡无重叠、无溢出且只有必要的一行换行。判定现按真实行高区分一行与两行，Chrome／WebKit 的 32 种布局矩阵复跑通过，未改变产品 CSS 或降低重叠／溢出／短消息同行断言。
...
- [PR 完整 CI](https://github.com/zdaiwmm/shui-IM/actions/runs/33905364719) 与[精确 main 完整 CI](https://github.com/zdaiwmm/shui-IM/actions/runs/33905829879) 均通过全部完整作业；应用提交 `e619afb8f9f732839872815c591ea56e454f40e3` 已经固定入口发布，`DEPLOY_OK`／`DEPLOY_VERIFIED` 与 02:32:39 独立回读均匹配。公开构建产物逐字节一致，生产证据见[发布记录](./RELEASING.md#本次线上发布记录)和[状态页](./docs/context/status.md)。
...
- `tests/chat-list-viewport.e2e.mjs`：在触屏 WebKit 夹具中明确启用 iPhone 独立滚动路径，检查长列表、历史锚点、历史位置聚焦、首次输入与删除、跨帧组词不临时缩行、六行草稿滚动后的固定留白、长草稿下列表贴底、键盘共享中间位置与迟到端点不二次位移、保险箱往返、引用跳转、到底部、320px／横屏布局和锁定清理。聊天主动滚动不能改变页面根位置，根不保留旧滚动预留区；列表保留惯性和边界回弹。原生根视口平移、标题闪动及键盘／URL 栏运动时序必须另外由 iPhone / iOS 27 / Safari 实测，桌面 WebKit 不能替代该证据。
...
- 多行输入专项在聚焦输入框及模拟已打开/平移键盘视口下逐帧采样：标题、照片按钮和发送/麦克风底边稳定；输入框向上伸展，列表与输入栏保持相对间距；发送后收缩且原消息连续上移。补测组词途中反复换行、伸展未结束立即发送、阅读旧历史时换行、伸展途中离页后的高度收尾及减少动态效果，不能在动画终点反向回跳。`tests/chat-bottom-control.e2e.mjs` 分别使用默认 Chromium 和 `QUIET_ROOM_TEST_BROWSER=webkit` 执行；两者都不是原生 iPhone 键盘验收。
...
5. 单柄对勾“已发送”（完整说明：服务器已保存）只依据提交 ACK/同步结果；双柄对勾“已读”依据对方角色发出的、绑定精确目标的加密已读事件。对端 ECDSA 签名接收回执仍用于内部送达事实，不能单独升级为双勾。
6. WebSocket 使用应用层心跳发现半开连接，退避重连后自动补消息、补回执、补待发箱。
7. 加密原图按 2 MiB 分块上传；中断后使用原 `blobId`、密钥和 IV 前缀续传，只补缺失分块；完成前不可下载。
8. 每台设备使用独立身份、MLS leaf 和服务端令牌；参与者邀请与新设备链接使用不同且严格校验的 fragment/schema，第二次新增设备仍必须进入设备审批流程，不得误报“邀请已被另一位参与者使用”。新设备只有经过原设备核对六位安全码并提交 MLS Add 后才激活，并从激活边界后的消息开始同步。
...
12. 可视区附近图片自动下载、校验、解密为默认模糊的内联缩略图，覆盖聊天单图、相册格和视频预览；含留白的竖图／横图必须以同一份已验证对象 URL 的 cover 模糊层填满背景，不得出现模糊断层或异色条。首次点击只显露当前图，再次点击打开。聊天、相册和查看器在同一次解锁期间复用已验证缓存。相册初次载入及缓存解码阶段使用整格柔光骨架与无障碍状态，减少动态效果时静态展示，失败保留重试；锁定清空解密缓存和对象 URL。
13. 点按已显露的单图或相册格从正确索引打开 overlay；前后按钮、触摸横划、键盘方向键、计数和关闭焦点恢复一致；聊天跨本地已加载消息浏览全部照片和视频，排除表情、普通文件及不可见消息。聊天查看器无下载按钮，收藏反馈在查看器内可见；保险箱查看器无收藏按钮。图片层无装饰内边距，按比例播满视口宽度，仅在高度先达到视口边界时允许水平留白；查看器背景须使用当前日间／夜间页面背景且不得添加图片高斯模糊层。正常缩放范围为默认适配尺寸的 1/3×–5×；越界手势有阻尼并在松手后回弹。下拉关闭过程随距离增加阻尼并逐渐缩小图片。横向分页必须跟手且有阻尼，松手后使用双层 380ms 平滑交接，顶部标题不重建或闪烁；双击放大／复位经过逐帧 transform 动画。聊天、保险箱和收藏中从图片切到视频时立即请求系统播放器与播放，不先展示页面视频详情；浏览器拒绝原生入口时保留原生控件回退并可横划到后续图片；离开视频立即暂停并卸载 source。切页不得重排聊天或产生新消息。聊天向下拖拽或查看器下拉关闭后隐藏全部聊天缩略图；隐私遮盖层同步清除显露状态并关闭查看器，包括移动端短暂失焦后快速恢复、缓存重建和迟到解码。拖拽后的合成点击不得再次显露。
...
5. 每次未归属原生前台交接的 `blur` 与 `visibilitychange(hidden)` 在同一事件调用栈内显示常驻不透明遮盖层，不等待下一帧；移动端/PWA 的无归属 `blur`、`hidden`、所有客户端的 `pagehide`、`freeze`、BFCache 恢复、手动/空闲锁定及未归属交接的系统组件失焦立即清理会话、WebSocket、对象 URL 和传输。已归属且未过期的键盘、选择器或恢复输入交接可以在判定后撤下遮挡，恢复输入的可见失焦最长 60 秒。桌面浏览器的普通失焦/隐藏立即遮蔽和清理运行状态，但仅内存保留解锁会话，30 分钟无应用内操作后真正锁定。仅会话和 socket 打开前的网关 WebAuthn 有有界隐藏豁免；回焦等待不覆盖原始失败，可见状态下超时丢弃结果并保留可重试错误。用户刚打开的选择器／媒体授权可有界接受可见 blur，见 D-020。v3 通行密钥失败不累计旧密码和手势的防猜测等待。
6. 回到前台不得自动恢复；右下角长按需显示克制的按住进度提示，达阈值后直接发起通行密钥验证，不播放完成扩散环／烟花，不等待或自动点击中间页按钮。取消只保留中性重试入口，不显示红色错误文案；真实失败可手动重试。v3 网关原生请求在锁定、pagehide、freeze 和 65 秒到期后须收到 AbortSignal，重复进入不残留 pending 请求；原生结果早于 Safari 回焦时须保持遮蔽，允许最长 1.5 秒的真实焦点返回后再继续，仍未聚焦则失败关闭。真实 Safari 的原生弹窗和面容流程须另外真机验证。
7. 聊天和相册选择器的首次前台可见失焦可由有界交接接管，返回焦点消费交接；若没有及时收到 `change`/`cancel`，或发生真实后台、第二次失焦、手动锁定、`pagehide`/freeze 或所有者改变，必须移除 hidden input、忽略迟到事件并要求用户解锁后重新选择；不保留或后台续传待选文件。浏览器代码只能尽力拆除 DOM 入口，不能保证关闭操作系统自己的弹窗或 tips。
8. 相册入口、列表、详情和直传仅创建者可用；一次可选多张，按顺序分别写入相册，均不进入双方聊天消息流。保险箱长按的置顶／删除仅改变该设备的分类投影，不重排或删除聊天气泡；加密偏好持久化失败必须回滚界面。
...
3. 输入器不提供独立表情面板、梗图入口和表情收藏功能；消息长按菜单提供六种快速回应，每个参与者同一消息最多一个回应，可替换或取消，回应显示在气泡角上且不新增聊天行。加入或移除回应后，后续气泡使用有界 FLIP 位移平滑落到新位置，不得瞬间跳动。
4. 约 500ms 长按显示表情条和纵向拷贝/选择文字/回复/删除菜单，删除文字和图标均为红色，点击后二级菜单按权限显示“为所有人删除”与／或“仅为我删除”。本机删除只保存加密隐藏偏好；全员删除通过能力门禁的加密 tombstone 投影，不宣称撤回离线端已读明文或物理擦除备份。背景轻微模糊、选中消息清晰；320px 长消息下控件不重叠。选择文字直接在原气泡内选中只读文本，位置与尺寸不跳动，不出现弹窗；允许系统调整选区，回执更新保留正在选择的 DOM。原生工具栏与拖动手柄需真机确认。
5. 图片入口与有界前台选择器交接保留键盘意图；真实后台以遮蔽锁定为先。键盘打开时列表垂直触摸不得滚动或在移动中收键盘，最后一指松开才收起；输入栏之外点按在松开时收键盘。
6. `VisualViewport` 高度/偏移变化后底部输入栏保持在可见键盘上沿；滚动、键盘弹出或收起动效的首帧立即隐藏输入栏，几何连续稳定 160ms 后再从下方 14px 以 280ms 渐显上滑返回，焦点、草稿和选区不丢失。双击、移动端 gesture 和修饰键滚轮不放大整页，图片放大只在专用 overlay 内发生。
7. 聊天使用页面原生滚动，真实消息内容延伸到网页控件与浏览器界面后方；标题栏和输入栏固定在 visual viewport，控件使用独立、有界的玻璃滤镜、方向高光与明暗边缘，整条栏不重复模糊。页面切换不发生位移抖动，查看器全屏展示并支持拖拽缩小退出，气泡内时间／状态文字使用比正文弱的灰色，尊重 `prefers-reduced-motion`。
8. 向左拖拽已确认气泡通过阻尼距离进入回复就绪状态，松手返回原位并自动聚焦键盘；键盘已打开时仍能滑动回复，不被列表收键手势抢占。引用与输入框共用一个外轮廓，摘要最多一行，长文本省略时不挤压关闭控件。
9. 普通网页只能通过透明 `theme-color`、`viewport-fit=cover` 和 PWA display 配置尽力融合系统界面，不能强制 Safari/Chrome 原生底部工具栏透明；验收不得把浏览器自行保留的不透明原生栏误判为 CSS 回归。
10. 新 WebSocket 会话默认 `away`；只有设备实际在聊天页时上报 `chat`。同一角色任一活跃设备为 `chat` 即该角色在线，其余设备 `away` 或关闭不能错误拉低聚合结果。
11. 进入相册、图片 overlay 或设备管理应即时变为 `away`，返回聊天即时变为 `chat`，全过程不主动关闭/重建 WebSocket；重连鉴权完成后重放最后期望状态。
12. presence 帧严格限制为 `chat|away`，服务端在 socket close、成员撤销和控制帧 ping/pong 超时后重算并广播。状态只在服务端内存，重启归零；UI 必须把它描述为聊天页面行为提示，不得当作端到端认证、本人注视或送达证明。
13. 加载失败页保留右下角空白长按入口，错误码后显示同字体且略微突出的未读数。计数只包含对方、激活边界之后、尚未在本机聊天可视区标记已读的聊天消息；相册直传、表情回应及同角色其他设备消息不计。锁定/刷新后仅凭独立只读令牌查询数字，前台每 15 秒与 online/focus 刷新；离线保留最近已确认值。只读令牌不得访问 room、WS、附件或推进已读，撤销/恢复/轮换后失效。
14. 表情目标、emoji 和取消事件在加密载荷内严格校验。跨设备按服务器序合并，每角色只保留最新回应；重复/待发确认不重复渲染。重开会话、目标分页、损坏缓存页和锁定中断扫描仍保持历史边界；旧设备未确认能力时禁止发送。
...
## 2026-09-05 录屏交互补齐与附件保险箱验收

- 冻结工作树 `npm run check:full` 从头通过：51 个 Vitest 文件、402 项单元／集成测试，
  以及 23／23 个顺序浏览器入口；最终产品修正后的浏览器汇总为 223.86 秒。构建仅保留
  既有第三方注释和大包
  警告，没有类型或打包错误。
- 认证专项覆盖：长按遮蔽入口在可信手势栈内立即发起 WebAuthn；系统验证结果结算前不占用
  跨标签保险库锁；第一次验证迟到时，显式锁定后的第二次验证仍可出现，旧成功不能覆盖新忙碌态。
- 键盘／动效专项覆盖：视口连续帧只同步固定标题栏和输入栏，160ms 稳定点后再校正消息；
  引用关闭保留既有键盘；页面以 380ms 双层推入／推出且无空白帧；消息和保险箱 tips 使用
  320ms 锚点缩放与对称关闭，动画中的可见触控项仍不小于 44px。
- 附件专项覆盖：聊天文档自动出现在创建者保险箱文件列表，照片／视频归相册；保险箱直传
  仍不进入聊天，单独恢复仍不产生聊天行或回复目标。PDF 在点击的可信激活栈内预留隔离
  `opener` 的系统阅读窗口，完整校验后才导航到本地 `blob:`；真实浏览器链路用纯文本
  验证无插件阅读器，PDF 与其他惰性类型由白名单单测覆盖；未知二进制仍按原字节下载，
  锁定、失败、取消会关闭未完成窗口，本地删除投影保持隔离。
- 媒体专项覆盖：单图／视频气泡、预览容器和缩略图几何一致；聊天、保险箱、查看器的遮蔽
  状态保持但模糊减弱；极端宽高比不失真；查看器图片无装饰内边距地按比例铺满，背景跟随日间／夜间主题且无图片高斯模糊；缩放可至默认尺寸 1/3 并带阻尼
  回弹，下拉边缘拖拽有阻尼且逐渐缩小；视频海报遇到首个未解码黑帧时有界改取后续帧，原视频逐字节不变。
- 语音与手势专项覆盖：语音气泡全区域长按、文件交互不缩小、左滑只在松手后激活回复、
  删除二级菜单无预选态；录音请求不再强制单声道／处理约束，以兼容蓝牙输入设备。自动化使用
  合成麦克风和桌面浏览器，仍不等于真实蓝牙耳机、iPhone／Android 键盘、系统 tips、原生
  播放器或触觉反馈的真机验收。

## 2026-09-06 语音拖拽松手反馈验收

- 隔离工作树的 `npm run check` 通过：51 个 Vitest 文件、413 项单元／集成测试；构建只有
  既有第三方注释与大包警告。第一次受限沙箱执行中，22 项本机服务测试仅因禁止监听
  `127.0.0.1` 失败；在允许本机监听的相同源码上原样重跑后 413／413 通过，没有修改产品或
  测试断言。
- `tests/voice-gestures.e2e.mjs` 在本机 Chrome 合成麦克风环境通过：取消阈值由 156 延长至
  220 CSS 像素，按钮使用更长的连续水平阻尼；阈值前保持蓝色并显示“松手发送”，越过后变红
  并显示“松手取消”，回拖至回差区外恢复发送。越过阈值时录音和音轨仍存活，只有取消态松手
  后才同步清理音轨与内存草稿，再播放退出动效。
- 专项同时复跑 320／390／1280 像素、明暗主题、减少动态效果、指针取消、授权迟到、暂停／
  试听／继续、硬件中断、时长上限、隐私清理与手势重绑；已目检 320 与 390 像素红蓝状态截图，
  按钮不遮挡结果文案；长按、免手持与取消退出期间均隐藏不可用的文字输入表面，录音关闭后
  恢复。没有运行完整浏览器套件、CI 或修复后的 iPhone／Android 真机验收，
  自动化不能证明真实设备阻尼手感。
...
- `tests/media-upload.e2e.mjs` 使用真实本机加密待发箱和分块密文，验证单图、相册、普通表情及自动隐藏表情在上传完成前插入气泡，局部百分比和失败重试，重试 ID 与原始字节不变，自动隐藏策略跨重试保持，100% 不冒充服务器确认，回执只更新装饰，本机提交失败恢复、页面切换和锁定 URL 清理。
- `tests/video-upload.e2e.mjs` 验证视频本地模糊封面、真实分块进度、上传中消息可见、同 ID 续传与锁定清理。两项均进入 `npm run check:full`；WebKit 模拟与真实 iPhone 验收分别记录。

## 2026-09-15 保险箱需求复验

- `gallery-loading.e2e.mjs`：阻塞本机历史解密时先显示整屏骨架；超过 200 条的记录扫描后，相册/文件不切 tab 即显示完整计数；新旧聊天附件均有独立 Safe 密文行；本机/全员聊天删除后 Safe 仍能查看；Safe 图片删除确认、Escape、持久化失败重试、计数更新和聊天保留。
