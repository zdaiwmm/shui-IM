...
- 后续回归：滚动条隐藏仍可原生滚动、鼠标／触摸上拉全屏、全屏焦点限制、动态主题色与关闭后恢复；长按菜单必须先把焦点交给已连接查看器再移除，迟到 contextmenu 不重开菜单。手机复测偶发进入遮蔽层；自动化验证 DOM 交接与原有离开即遮蔽边界，不证明已重现该偶发真机现象。Safari 系统栏以页面同色边缘和 theme-color 提示适配，不承诺强制透明。
- 视觉反馈回归：保险箱查看器无下载入口，聊天原有下载保留；信息按钮位于顶栏，详情沿用玻璃变量。专项覆盖外围点击、鼠标/真实触摸下拉、内容滚动、520ms 连续下行动画、明暗窄屏截图，以及进入保险箱时退场聊天图片逐帧不可见。真机仍需复核玻璃观感、手势手感和曾报告的闪图。
