...
| 图片相册与本机偏好 | 2–9 张原图为一条加密相册消息；可视区本机解密预览；overlay 翻页；阅读锚点与恢复提示偏好本机加密 | 严格 payload 单测、真实浏览器单消息/拼图/翻页/定位/拖拽关闭断言 | 大图内存压力、弱网、真机图片选择器与 iOS 返回动效验证 |
...
5. “已保存”（完整说明：服务器已保存）只依据提交 ACK/同步结果；“已送达”（完整说明：对端已安全接收）只依据对端 ECDSA 私钥签出的、与房间/消息 ID/序号绑定的回执。
...
3. 不提供表情面板、梗图入口和表情收藏功能；重开会话后也不得重新出现这些控件。
4. 聊天气泡禁用系统长按选择，约 500ms 后显示回复、复制和选择文字菜单；选择文字允许只复制选中范围，支持的设备提供轻震动反馈。
5. 输入框已聚焦时点图片入口和选择图片，不主动收起软键盘；点输入栏以外的空白区域收起键盘。
6. `VisualViewport` 高度/偏移变化后底部输入栏保持在可见键盘上沿；双击、移动端 gesture 和修饰键滚轮不放大整页，图片放大只在专用 overlay 内发生。
7. 消息滚动区域延伸到标题栏和输入栏下方；两个栏使用半透明渐变遮罩。页面切换不发生位移抖动，查看器全屏展示并支持拖拽缩小退出，尊重 `prefers-reduced-motion`。
...
- 图片入口保持输入焦点，外部空白收起键盘；模拟 `VisualViewport` 后输入栏仍在可见区；
...
- 置顶恢复提示可关闭并持久保存偏好；准备恢复后立即显示独立恢复码，下载并确认才记录完成；恢复文件与独立恢复码、新设备凭据重绑定、恢复后继续接收新 MLS 消息；
...
- VoiceOver/TalkBack 对在线状态、相册计数、overlay 控件、选择文字面板和 reduced-motion 的播报与焦点顺序；
