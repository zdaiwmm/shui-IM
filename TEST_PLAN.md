...
- iPhone 固定定位原点回归须独立模拟原生原点与延迟的 `visualViewport.offsetTop`：覆盖换行期间 21／23 px 偏移、发送期间 48／49 px 偏移，以及失焦后原点先归零而视口仍停留在键盘端点。无 resize／scroll 事件的下一渲染帧标题须保持顶边、输入栏布局底边须保持可视高度；14 px 显现动画不得被写入持久定位。桌面 WebKit 的合成坐标夹具不等于 iPhone 真机验收。
