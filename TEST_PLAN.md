...
13. 点按已显露的单图或相册格从正确索引打开 overlay；前后按钮、触摸横划、键盘方向键、计数、当前原图下载和关闭焦点恢复一致。横向分页必须跟手且有阻尼，松手后使用双层 380ms 平滑交接，顶部标题不重建或闪烁；双击放大／复位经过逐帧 transform 动画。从图片切到视频时视频默认暂停，仍可横划到后续图片；离开视频立即暂停并卸载 source。切页不得重排聊天或产生新消息。聊天向下拖拽或查看器下拉关闭后隐藏全部聊天缩略图；隐私遮盖层同步清除显露状态并关闭查看器，包括移动端短暂失焦后快速恢复、缓存重建和迟到解码。拖拽后的合成点击不得再次显露。
...
- 媒体专项覆盖：单图／视频气泡、预览容器和缩略图几何一致；聊天、保险箱、查看器的遮蔽
  状态保持但模糊减弱；极端宽高比不失真；查看器按比例铺满并使用弱化背景填充；缩放带阻尼
  回弹；视频海报遇到首个未解码黑帧时有界改取后续帧，原视频逐字节不变。
